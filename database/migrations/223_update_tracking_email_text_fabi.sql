-- Atualiza o TEXTO (assunto e corpo_html) dos e-mails de rastreio para o texto oficial da
-- Braziliana: saudação, orientações sobre impostos, passo a passo do portal Minhas Importações
-- dos Correios, alertas e assinatura "Love, Fabi". Mantém o layout padronizado (cabeçalho
-- escuro, corpo branco, box de rastreio, botão, rodapé).
--
...
-- Corpo HTML compartilhado. {URL_BTN} é trocado por REPLACE antes de aplicar;
-- placeholders {{...}} são resolvidos no momento do envio.
SET @corpo_rastreio := CONCAT(
