-- Inserir chave de configuração para webhook de link de pagamento de pedidos manuais
...
SET @sql1 := IF(
    ...
    CONCAT(
        ...
        'SELECT ''pagamentos'', ''webhook_link_pagamento_pedido_manual_url'', '''', NOW()\n',
        'WHERE NOT EXISTS (SELECT 1 FROM ', @cfg_table, ' WHERE categoria=''pagamentos'' AND chave=''webhook_link_pagamento_pedido_manual_url'');'
    ),
    'SELECT 1;'
);
...
SET @sql2 := IF(
    ...
    CONCAT(
        ...
        'SELECT ''pagamentos_webhook_link_pagamento_pedido_manual_url'', ''''', IF(@has_updated_at>0, ', NOW()', ''), '\n',
        'WHERE NOT EXISTS (SELECT 1 FROM ', @cfg_table, ' WHERE ', @key_col, '=''pagamentos_webhook_link_pagamento_pedido_manual_url'');'
    ),
    'SELECT 1;'
);
