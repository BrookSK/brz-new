...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
        <div class="d-flex align-items-center gap-3">
            <div>
                <h1 class="page-title"><?= __('admin.report_general.page_title','Financeiro') ?></h1>
                <p class="page-subtitle"><?= __('admin.report_general.page_subtitle','Visão consolidada de pedidos, receitas e impostos') ?></p>
            </div>
        </div>
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <button class="btn btn-outline-secondary btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#modalViewConfig"><i class="fas fa-globe me-1"></i><span class="d-none d-md-inline"><?= __('admin.report_general.currency','Moeda') ?></span></button>
            <button class="btn btn-outline-secondary btn-sm rounded-pill px-3" onclick="exportarDRE()"><i class="fas fa-download me-1"></i><span class="d-none d-md-inline"><?= __('admin.report_general.export','Exportar') ?></span></button>
        </div>
    </div>
    ...
    <div class="mb-4">
        <ul class="nav nav-pills gap-2 flex-nowrap overflow-auto" role="tablist" style="-webkit-overflow-scrolling:touch;">
            <li class="nav-item flex-shrink-0"><button class="nav-link active px-3 py-2" id="tab-geral" data-bs-toggle="tab" data-bs-target="#pane-geral" type="button"><i class="fas fa-chart-bar me-1"></i><span class="d-none d-sm-inline"><?= __('admin.report_general.tab_general_prefix','Relatório ') ?></span><?= __('admin.report_general.tab_general','Geral') ?></button></li>
            <li class="nav-item flex-shrink-0"><button class="nav-link px-3 py-2" id="tab-regional" type="button" onclick="abrirModalRegional()"><i class="fas fa-globe me-1"></i><?= __('admin.report_general.tab_regional','Regional') ?></button></li>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <div class="d-flex align-items-center gap-2"><i class="fas fa-sliders-h text-muted"></i><span class="fw-semibold"><?= __('admin.report_general.report_filters','Filtros do relatório') ?></span></div>
                <a href="/admin/relatorio-geral" class="text-muted small text-decoration-none"><i class="fas fa-times me-1"></i><?= __('admin.report_general.clear_filters','Limpar filtros') ?></a>
            </div>
            <form method="GET" action="/admin/relatorio-geral">
                <div class="d-flex align-items-end flex-wrap gap-3">
                    <!-- Datas -->
                    <div class="flex-shrink-0">
                        <label class="form-label small text-muted mb-1"><i class="far fa-calendar me-1"></i><?= __('admin.report_general.date_start','DATA INÍCIO') ?></label>
                        <input type="date" name="date_start" class="form-control form-control-sm" value="<?= htmlspecialchars($dateStart) ?>">
                    </div>
                    <div class="flex-shrink-0">
                        <label class="form-label small text-muted mb-1"><i class="far fa-calendar me-1"></i><?= __('admin.report_general.date_end','DATA FIM') ?></label>
                    ...
                    <div class="flex-grow-1">
                        <label class="form-label small text-muted mb-1"><i class="fas fa-tag me-1"></i><?= __('admin.report_general.order_status','STATUS DO PEDIDO') ?></label>
                        <div class="d-flex flex-wrap gap-1" id="statusChipsContainer">
                            <?php
                            $statusTooltips = [
                                'itens_parcialmente_comprados' => __('admin.report_general.tooltip_itens_parcialmente_comprados','Pedido pago com parte dos itens já comprados do fornecedor. Itens restantes ainda pendentes de compra.'),
                                'itens_comprados' => __('admin.report_general.tooltip_itens_comprados','Todos os itens do pedido foram comprados do fornecedor.'),
                                'produto_consolidado' => __('admin.report_general.tooltip_produto_consolidado','Caixa fechada — engloba Pago, Parcialmente Comprado e Produto Comprado.'),
                                'pago' => __('admin.report_general.tooltip_pago','Pagamento confirmado. Itens aguardando compra do fornecedor.'),
                            ];
                    ...
                    <!-- Moeda -->
                    <div>
                        <label class="form-label small text-muted mb-1"><i class="fas fa-coins me-1"></i><?= __('admin.report_general.currency_upper','MOEDA') ?></label>
                        <select name="moeda" class="form-select form-select-sm" style="min-width:100px;">
                            <option value=""><?= __('admin.report_general.all_f','Todas') ?></option>
                            <option value="BRL" <?= $moedaFilter === 'BRL' ? 'selected' : '' ?>><?= __('admin.report_general.brl_real','BRL — Real') ?></option>
                            <option value="USD" <?= $moedaFilter === 'USD' ? 'selected' : '' ?>><?= __('admin.report_general.usd_dollar','USD — Dólar') ?></option>
                        </select>
                    </div>

                    <!-- Filtrar -->
                    <div>
                        <button type="submit" class="btn btn-dark btn-sm px-4 py-2"><i class="fas fa-filter me-1"></i><?= __('admin.report_general.filter','Filtrar') ?></button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4" style="background:linear-gradient(135deg,#1e293b 0%,#334155 100%);">
        <div class="card-body d-flex align-items-center justify-content-between py-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <div class="text-white fw-semibold text-uppercase small"><?= __('admin.report_general.total_orders','Total de pedidos') ?></div>
                    <div class="text-white text-opacity-75 small"><?= __('admin.report_general.period_label','Período: {p}', ['p' => $periodoLabel]) ?></div>
                </div>
            </div>
            <div class="text-end">
    ...
    <?php
    $campos = [
        ['key'=>'total','label'=>__('admin.report_general.total_general','Total geral'),'icon'=>'fas fa-dollar-sign','color'=>'primary'],
        ['key'=>'subtotal','label'=>__('admin.report_general.subtotal_products','Subtotal produtos'),'icon'=>'fas fa-box','color'=>'success'],
        ['key'=>'servicos','label'=>__('admin.report_general.service_fee','Taxa de serviço'),'icon'=>'fas fa-concierge-bell','color'=>'info'],
        ['key'=>'impostos','label'=>__('admin.report_general.taxes','Impostos'),'icon'=>'fas fa-landmark','color'=>'warning'],
        ['key'=>'imposto_local','label'=>__('admin.report_general.local_tax','Imposto local'),'icon'=>'fas fa-flag','color'=>'danger'],
        ['key'=>'frete','label'=>__('admin.report_general.shipping','Frete'),'icon'=>'fas fa-truck','color'=>'secondary'],
    ];
        ...
        <div class="card border-0 shadow-sm h-100" style="border-top:3px solid <?= $borderColor ?>;">
            <div class="card-body">
                ...
                <div class="text-center py-3">
                    ...
                    <div class="text-muted small fst-italic"><?= __('admin.report_general.no_values_of','Sem valores de {label} no período', ['label' => strtolower($c['label'])]) ?></div>
                </div>
                <?php else: ?>
                    <?php if ($vUsd > 0): ?>
                    ...
                    <div class="mt-3 pt-2 border-top d-flex align-items-baseline justify-content-between">
                        <span class="fw-bold small text-uppercase" style="color:<?= $borderColor ?>;font-size:11px;" data-i18n="total_em_brl"><?= __('admin.report_general.total_in_brl','Total em BRL') ?></span>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-semibold small"><?= __('admin.report_general.total_weight_kg','Peso total (KG)') ?></span>
                        </div>
                        <i class="fas fa-info-circle text-muted" style="font-size:12px;" title="<?= htmlspecialchars(__('admin.report_general.tooltip_total_weight','Soma do peso de todos os pedidos filtrados'), ENT_QUOTES, 'UTF-8') ?>" data-bs-toggle="tooltip"></i>
                    </div>
                    <?php if ($pesoTotal > 0): ?>
                    ...
                    <div class="text-center py-3">
                        ...
                        <div class="text-muted small fst-italic"><?= __('admin.report_general.no_weight_data','Sem dados de peso no período') ?></div>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-semibold small"><?= __('admin.report_general.discounts_granted','Descontos concedidos') ?></span>
                        </div>
                        <i class="fas fa-info-circle text-muted" style="font-size:12px;" title="<?= htmlspecialchars(__('admin.report_general.tooltip_discounts','Total de descontos aplicados nos pedidos filtrados. Lançado como despesa operacional.'), ENT_QUOTES, 'UTF-8') ?>" data-bs-toggle="tooltip"></i>
                    </div>
                    <?php if ($descontoUsd > 0 || $descontoBrl > 0): ?>
                        ...
                        <?php endif; ?>
                        <div class="mt-3 pt-2 border-top d-flex align-items-baseline justify-content-between">
                            <span class="fw-bold small text-uppercase" style="color:#f43f5e;font-size:11px;"><?= __('admin.report_general.total_in_brl','Total em BRL') ?></span>
                    ...
                    <div class="text-center py-3">
                        ...
                        <div class="text-muted small fst-italic"><?= __('admin.report_general.no_discounts','Sem descontos no período') ?></div>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-semibold small"><?= __('admin.report_general.discount_authorizations','Autorizações de desconto') ?></span>
                        </div>
                    </div>
                    <div class="text-center">
                        <span class="fs-3 fw-bold" style="color:#f97316;"><?= count($descontoDetalhe) ?></span>
                        <div class="text-muted small"><?= __('admin.report_general.in_period','no período') ?></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4" style="border-top:3px solid #f43f5e;">
        <div class="card-header bg-white border-0 pt-3">
            <div class="d-flex align-items-center gap-2">
                ...
                <div><div class="fw-bold small"><?= __('admin.report_general.discount_breakdown','Detalhamento de Descontos') ?></div><div class="text-muted" style="font-size:10px;"><?= __('admin.report_general.discount_breakdown_sub','Discriminação por produto/pedido, agente e valores') ?></div></div>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-sm table-hover mb-0" style="font-size:12px;">
                    <thead class="table-light">
                        <tr>
                            <th><?= __('admin.report_general.th_product_order','Produto / Pedido') ?></th>
                            <th class="text-end"><?= __('admin.report_general.th_original_value','Valor Original') ?></th>
                            <th class="text-end"><?= __('admin.report_general.th_discount','Desconto') ?></th>
                            <th><?= __('admin.report_general.th_agent','Agente (Vendedor)') ?></th>
                            <th class="text-end"><?= __('admin.report_general.th_final_value','Valor Final') ?></th>
                            <th><?= __('admin.report_general.th_date','Data') ?></th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php
                    ...
                    <tfoot class="border-top fw-bold">
                        <tr>
                            <td><?= __('admin.report_general.total_items','Total ({n} itens)', ['n' => count($descontoDetalhe)]) ?></td>
            ...
            <div class="card border-0 shadow-sm" style="border-top:3px solid #1e293b;">
                <div class="card-header bg-white border-0 pt-3 d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><h6 class="fw-bold mb-0"><?= __('admin.report_general.dre_title','DRE — Demonstrativo de Resultado') ?></h6><span class="text-muted" style="font-size:10px;"><?= __('admin.report_general.dre_subtitle','Receitas vs Despesas no período') ?></span></div>
                    </div>
                    <a href="/admin/despesas" class="btn btn-sm btn-outline-secondary rounded-pill"><i class="fas fa-external-link-alt me-1"></i><?= __('admin.report_general.view_expenses','Ver despesas') ?></a>
                </div>
                <div class="card-body">
                    <div class="row g-4">
                        <div class="col-lg-5">
                            <table class="table table-sm mb-0" style="font-size:13px;">
                                <tbody>
                                    <tr class="border-bottom"><td class="fw-bold text-success" data-i18n="receita_bruta"><i class="fas fa-arrow-up me-1"></i><?= __('admin.report_general.gross_revenue','RECEITA BRUTA') ?></td><td class="text-end fw-bold text-success fs-5 fin-value" data-value-brl="<?= $receitaBruta ?>"><?= fmtNum($receitaBruta) ?></td></tr>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.subtotal_products','Subtotal produtos') ?></td><td class="text-end"><?= fmtNum($totalSubtotal) ?></td></tr>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.service_fee','Taxa de serviço') ?></td><td class="text-end"><?= fmtNum($totalServicos) ?></td></tr>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.taxes_charged','Impostos cobrados') ?></td><td class="text-end"><?= fmtNum($totalImpostos) ?></td></tr>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.shipping','Frete') ?></td><td class="text-end"><?= fmtNum($totalFrete) ?></td></tr>
                                    <tr><td class="ps-3 <?= $descontoTotalBrl > 0 ? 'text-danger' : 'text-muted' ?>"><i class="fas fa-percent me-1" style="font-size:10px;"></i><?= __('admin.report_general.discounts_granted','Descontos concedidos') ?></td><td class="text-end <?= $descontoTotalBrl > 0 ? 'text-danger' : '' ?>"><?= $descontoTotalBrl > 0 ? '-' : '' ?><?= fmtNum($descontoTotalBrl) ?></td></tr>
                                    <tr class="border-top border-bottom"><td class="fw-bold text-danger" data-i18n="despesas_totais"><i class="fas fa-arrow-down me-1"></i><?= __('admin.report_general.total_expenses','DESPESAS TOTAIS') ?></td><td class="text-end fw-bold text-danger fs-5 fin-value" data-value-brl="<?= $totalDespesas ?>"><?= fmtNum($totalDespesas) ?></td></tr>
                                    <?php if ($despUsd > 0): ?>
                                    ...
                                    <?php endif; ?>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.paid_in_period','Pagas no período') ?></td><td class="text-end"><?= fmtNum($despesasResumo['pago']) ?></td></tr>
                                    <tr><td class="ps-3 text-muted"><?= __('admin.report_general.outstanding','Em aberto') ?></td><td class="text-end"><?= fmtNum($despesasResumo['aberto']) ?></td></tr>
                                    <tr class="border-top" style="background:#f8fafc;"><td class="fw-bold" style="font-size:14px;" data-i18n="resultado"><i class="fas fa-equals me-1"></i><?= __('admin.report_general.net_result','RESULTADO LÍQUIDO') ?></td><td class="text-end fw-bold fs-4 <?= $lucroLiquido >= 0 ? 'text-success' : 'text-danger' ?> fin-value" data-value-brl="<?= $lucroLiquido ?>"><?= fmtNum($lucroLiquido) ?></td></tr>
                                    <tr><td class="text-muted small"><?= __('admin.report_general.margin','Margem') ?></td><td class="text-end"><span class="badge <?= $margemLucro >= 0 ? 'bg-success' : 'bg-danger' ?>"><?= $margemLucro ?>%</span></td></tr>
                                </tbody>
                            </table>
                        </div>
                        <div class="col-lg-7">
                            <h6 class="fw-bold small mb-2"><i class="fas fa-chart-pie me-1 text-muted"></i><?= __('admin.report_general.expenses_by_category','Despesas por Categoria') ?></h6>
                            <?php if (empty($despesasResumo['por_categoria'])): ?>
                            <div class="text-center text-muted py-4 small"><i class="fas fa-inbox d-block mb-2" style="font-size:24px;opacity:.4;"></i><?= __('admin.report_general.no_expenses_registered','Nenhuma despesa registrada no período.') ?><br><a href="/admin/despesas" class="mt-2 d-inline-block"><?= __('admin.report_general.register_expenses','Cadastrar despesas →') ?></a></div>
                            <?php else: ?>
                            <div class="table-responsive">
                                <table class="table table-sm mb-0" style="font-size:12px;">
                                    <thead class="table-light"><tr><th><?= __('admin.report_general.th_category','Categoria') ?></th><th><?= __('admin.report_general.th_group','Grupo') ?></th><th class="text-end"><?= __('admin.report_general.th_qty','Qtd') ?></th><th class="text-end"><?= __('admin.report_general.th_total_brl','Total (R$)') ?></th><th>%</th></tr></thead>
                                    <tbody>
                                    ...
                                    <?php endforeach; ?>
                                    <tr class="fw-bold border-top"><td><?= __('admin.report_general.total','Total') ?></td><td></td><td class="text-end"><?= array_sum(array_column($despesasResumo['por_categoria'], 'qtd')) ?></td><td class="text-end"><?= fmtNum($totalDespesas) ?></td><td></td></tr>
                                    </tbody>
                                </table>
                            </div>
                            <?php endif; ?>
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 pt-3 d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><div class="fw-bold small"><?= __('admin.report_general.by_status','Por Status') ?></div><div class="text-muted" style="font-size:10px;"><?= __('admin.report_general.values_grouped_brl','Valores agrupados em R$') ?></div></div>
                    </div>
                    <span class="text-muted small"><?= __('admin.report_general.view_details','Ver detalhes →') ?></span>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-sm mb-0" style="font-size:12px;">
                            <thead class="table-light"><tr><th><?= __('admin.report_general.th_status','Status') ?></th><th class="text-end"><?= __('admin.report_general.th_qty','Qtd') ?></th><th class="text-end"><?= __('admin.report_general.th_subtotal','Subtotal') ?></th><th class="text-end"><?= __('admin.report_general.th_service','Serviço') ?></th><th class="text-end"><?= __('admin.report_general.taxes','Impostos') ?></th><th class="text-end"><?= __('admin.report_general.shipping','Frete') ?></th><th class="text-end"><?= __('admin.report_general.th_total_brl','Total (R$)') ?></th></tr></thead>
                            <tbody>
                            <?php if (empty($statusConsolidado)): ?>
                                <tr><td colspan="7" class="text-center text-muted py-3"><?= __('admin.report_general.no_data','Nenhum dado') ?></td></tr>
                            <?php else: ?>
                                <?php foreach ($statusConsolidado as $row):
                                    $cor = $statusColors[$row['status']] ?? 'secondary';
                                    $statusDescricoes = [
                                        'itens_parcialmente_comprados' => __('admin.report_general.desc_itens_parcialmente_comprados','Parte dos itens já comprados, restantes pendentes'),
                                        'itens_comprados' => __('admin.report_general.desc_itens_comprados','Todos os itens comprados do fornecedor'),
                                        'produto_consolidado' => __('admin.report_general.desc_produto_consolidado','Caixa fechada (já pago e comprado)'),
                                        'pago' => __('admin.report_general.desc_pago','Pagamento confirmado, aguardando compra'),
                                    ];
                                ...
                                <?php endforeach; ?>
                                <tr class="fw-bold border-top"><td><?= __('admin.report_general.total','Total') ?></td><td class="text-end"><?= $totalQtd ?></td><td class="text-end"><?= fmtNum($totalSubtotal) ?></td><td class="text-end"><?= fmtNum($totalServicos) ?></td><td class="text-end"><?= fmtNum($totalImpostos) ?></td><td class="text-end"><?= fmtNum($totalFrete) ?></td><td class="text-end"><?= fmtNum($totalTotal) ?></td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 pt-3">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><div class="fw-bold small"><?= __('admin.report_general.by_currency','Por Moeda') ?></div><div class="text-muted" style="font-size:10px;"><?= __('admin.report_general.currency_distribution','Distribuição cambial') ?></div></div>
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-sm mb-0" style="font-size:12px;">
                            <thead class="table-light"><tr><th><?= __('admin.report_general.th_currency','Moeda') ?></th><th class="text-end"><?= __('admin.report_general.th_qty','Qtd') ?></th><th class="text-end"><?= __('admin.report_general.th_total','Total') ?></th></tr></thead>
                            <tbody>
                            ...
                            <?php else: ?>
                                ...
                                <?php endforeach; ?>
                                <tr class="fw-bold border-top"><td><?= __('admin.report_general.total','Total') ?></td><td class="text-end"><?= $totalMoedaQtd ?></td><td class="text-end"><?= fmtNum($totalMoedaTotal) ?></td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 pt-3">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><div class="fw-bold small"><?= __('admin.report_general.by_payment','Por Pagamento') ?></div><div class="text-muted" style="font-size:10px;"><?= __('admin.report_general.payment_method','Forma de pagamento') ?></div></div>
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-sm mb-0" style="font-size:12px;">
                            <thead class="table-light"><tr><th><?= __('admin.report_general.th_method','Forma') ?></th><th class="text-end"><?= __('admin.report_general.th_qty','Qtd') ?></th><th class="text-end"><?= __('admin.report_general.th_total','Total') ?></th></tr></thead>
                            <tbody>
                            ...
                            <?php else: ?>
                                ...
                                <?php endforeach; ?>
                                <tr class="fw-bold border-top"><td><?= __('admin.report_general.total','Total') ?></td><td class="text-end"><?= $totalPagQtd ?></td><td class="text-end text-success"><?= fmtNum($totalPagTotal) ?></td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body py-3">
                <div class="d-flex align-items-end gap-3 flex-wrap">
                    <div><label class="form-label small text-muted mb-1"><i class="far fa-calendar me-1"></i><?= __('admin.report_general.date_start_c','Data início') ?></label><input type="date" id="dre-date-start" class="form-control form-control-sm" value="<?= htmlspecialchars($dateStart) ?>"></div>
                    <div><label class="form-label small text-muted mb-1"><i class="far fa-calendar me-1"></i><?= __('admin.report_general.date_end_c','Data fim') ?></label><input type="date" id="dre-date-end" class="form-control form-control-sm" value="<?= htmlspecialchars($dateEnd) ?>"></div>
                    <div><button class="btn btn-dark btn-sm px-4" onclick="dreApplyFilter()"><i class="fas fa-filter me-1"></i><?= __('admin.report_general.filter_dre','Filtrar DRE') ?></button></div>
                    <div class="ms-auto"><button class="btn btn-outline-secondary btn-sm" onclick="dreExport()"><i class="fas fa-download me-1"></i><?= __('admin.report_general.export_csv','Exportar CSV') ?></button></div>
                </div>
            </div>
        </div>
        <div id="dre-completo-container">
            <div class="text-center py-5"><i class="fas fa-spinner fa-spin fs-3 text-muted"></i><div class="text-muted mt-2"><?= __('admin.report_general.loading_dre_full','Carregando DRE Completo...') ?></div></div>
        </div>
    </div><!-- /pane-dre -->
    </div>
</div>
...
<div class="modal fade" id="modalViewConfig" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title"><i class="fas fa-globe me-2"></i><?= __('admin.report_general.visualization','Visualização') ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="mb-3"><label class="form-label fw-semibold small"><?= __('admin.report_general.display_currency','Moeda de exibição') ?></label><select id="global-view-currency" class="form-select"><option value="BRL"><?= __('admin.report_general.brl_real_paren','BRL (Real)') ?></option><option value="USD"><?= __('admin.report_general.usd_dollar_paren','USD (Dólar)') ?></option></select></div>
                <div class="mb-0"><label class="form-label fw-semibold small"><?= __('admin.report_general.language','Idioma') ?></label><select id="global-view-lang" class="form-select"><option value="pt"><?= __('admin.report_general.lang_pt','Português (PT-BR)') ?></option><option value="en"><?= __('admin.report_general.lang_en','English (EN)') ?></option></select></div>
            </div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal"><?= __('admin.report_general.cancel','Cancelar') ?></button><button type="button" class="btn btn-primary btn-sm" onclick="applyGlobalView();bootstrap.Modal.getInstance(document.getElementById('modalViewConfig')).hide();"><i class="fas fa-check me-1"></i><?= __('admin.report_general.apply','Aplicar') ?></button></div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="modalRegional" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title"><i class="fas fa-globe me-2"></i><?= __('admin.report_general.regional_report','Relatório Regional') ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="mb-3"><label class="form-label fw-semibold small"><?= __('admin.report_general.display_currency','Moeda de exibição') ?></label><select id="regional-moeda" class="form-select"><option value="BRL"><?= __('admin.report_general.brl_real_paren','BRL (Real)') ?></option><option value="USD"><?= __('admin.report_general.usd_dollar_paren','USD (Dólar)') ?></option></select></div>
                <div><label class="form-label fw-semibold small"><?= __('admin.report_general.language','Idioma') ?></label><select id="regional-idioma" class="form-select"><option value="pt"><?= __('admin.report_general.lang_pt','Português (PT-BR)') ?></option><option value="en"><?= __('admin.report_general.lang_en','English (EN)') ?></option></select></div>
            </div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal"><?= __('admin.report_general.cancel','Cancelar') ?></button><button type="button" class="btn btn-primary btn-sm" onclick="gerarRelatorioRegional()"><i class="fas fa-check me-1"></i><?= __('admin.report_general.generate','Gerar') ?></button></div>
        </div>
    </div>
</div>
<script>
...
function updateEnglobadoVisual() {
    ...
    if (todosEnglobados.length > 0) {
        ...
        if (englobadosNomes.length > 0) {
            textEl.textContent = '<?= htmlspecialchars(__('admin.report_general.auto_includes','Inclui automaticamente: '), ENT_QUOTES, 'UTF-8') ?>' + englobadosNomes.join(', ');
...
window.gerarRelatorioRegional=function(){
    ...
    h+='<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0">'+t.byStatus+'</h6></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm mb-0" style="font-size:12px;"><thead class="table-light"><tr><th>'+t.status+'</th><th class="text-end">'+t.qty+'</th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_subtotal','Subtotal'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_service','Serviço'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.taxes','Impostos'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.shipping','Frete'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_total','Total'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>';
...
function loadDreCompleto() {
    ...
    fetch('/admin/dre-completo/dados?date_start=' + ds + '&date_end=' + de)
        .then(r => r.json())
        .then(d => { if (d.success) renderDreCompleto(d); else document.getElementById('dre-completo-container').innerHTML = '<div class="alert alert-danger"><?= htmlspecialchars(__('admin.report_general.error_loading_data','Erro ao carregar dados'), ENT_QUOTES, 'UTF-8') ?></div>'; })
        .catch(e => { document.getElementById('dre-completo-container').innerHTML = '<div class="alert alert-danger"><?= htmlspecialchars(__('admin.report_general.error_prefix','Erro: '), ENT_QUOTES, 'UTF-8') ?>' + e.message + '</div>'; });
}
...
function renderDreCompleto(d) {
    ...
    const gwNames = {'stripe':'Stripe','cambioreal':'Câmbio Real','cambioreal_taxas':'CR Taxas','mercadopago':'Mercado Pago','asaas':'Asaas','appmax':'AppMax','pagdev':'PagDev','sem_gateway':'<?= htmlspecialchars(__('admin.report_general.no_gateway','Sem gateway'), ENT_QUOTES, 'UTF-8') ?>','n/a':'N/A'};
    ...
    h += '<div class="row g-3 mb-4">';
    h += card('<?= htmlspecialchars(__('admin.report_general.total_income','Total Entradas'), ENT_QUOTES, 'UTF-8') ?>','fas fa-arrow-up','#10b981',fmtR(r.total_entradas));
    h += card('<?= htmlspecialchars(__('admin.report_general.income_orders','Entradas Pedidos'), ENT_QUOTES, 'UTF-8') ?>','fas fa-shopping-cart','#3b82f6',(r.qtd_pedidos||0)+' <?= htmlspecialchars(__('admin.report_general.orders_word','pedidos'), ENT_QUOTES, 'UTF-8') ?>');
    h += card('<?= htmlspecialchars(__('admin.report_general.total_expenses_c','Total Despesas'), ENT_QUOTES, 'UTF-8') ?>','fas fa-arrow-down','#ef4444',fmtR(r.total_despesas));
    h += card('<?= htmlspecialchars(__('admin.report_general.net_result_c','Resultado Líquido'), ENT_QUOTES, 'UTF-8') ?>','fas fa-equals',r.resultado>=0?'#10b981':'#ef4444',fmtR(r.resultado));
    h += card('<?= htmlspecialchars(__('admin.report_general.margin','Margem'), ENT_QUOTES, 'UTF-8') ?>','fas fa-percentage',r.margem>=0?'#3b82f6':'#ef4444',r.margem+'%');
    h += card('<?= htmlspecialchars(__('admin.report_general.income_usd','Entradas USD'), ENT_QUOTES, 'UTF-8') ?>','fas fa-dollar-sign','#6366f1','$ '+(r.total_entradas_usd||0).toLocaleString('en-US',{minimumFractionDigits:2}));
    h += card('<?= htmlspecialchars(__('admin.report_general.expenses_usd','Despesas USD'), ENT_QUOTES, 'UTF-8') ?>','fas fa-dollar-sign','#f59e0b','$ '+(r.total_despesas_usd||0).toLocaleString('en-US',{minimumFractionDigits:2}));
    h += card('<?= htmlspecialchars(__('admin.report_general.biggest_spending','Maior Gasto'), ENT_QUOTES, 'UTF-8') ?>','fas fa-fire','#dc2626',r.maior_categoria||'N/A');
    h += '</div>';

    // DRE Profissional
    h += '<div class="card border-0 shadow-sm mb-4" style="border-top:3px solid #1e293b;"><div class="card-header bg-white border-0 pt-3"><div class="d-flex align-items-center gap-2"><div class="rounded-circle bg-dark bg-opacity-10 d-flex align-items-center justify-content-center" style="width:28px;height:28px;"><i class="fas fa-file-invoice-dollar text-dark" style="font-size:11px;"></i></div><div><h6 class="fw-bold mb-0"><?= htmlspecialchars(__('admin.report_general.dre_balance','DRE — Balanço Financeiro'), ENT_QUOTES, 'UTF-8') ?></h6><span class="text-muted" style="font-size:10px;"><?= htmlspecialchars(__('admin.report_general.period_word','Período: '), ENT_QUOTES, 'UTF-8') ?>'+d.periodo.inicio+' <?= htmlspecialchars(__('admin.report_general.to_word','a'), ENT_QUOTES, 'UTF-8') ?> '+d.periodo.fim+'</span></div></div></div><div class="card-body"><table class="table table-sm mb-0" style="font-size:13px;"><tbody>';
    h += dreRow('<?= htmlspecialchars(__('admin.report_general.gross_operating_revenue','RECEITA OPERACIONAL BRUTA'), ENT_QUOTES, 'UTF-8') ?>',fmtR(r.total_entradas),'fw-bold text-success','fs-5');
    h += dreRow('  <?= htmlspecialchars(__('admin.report_general.income_in_brl','Entradas em BRL'), ENT_QUOTES, 'UTF-8') ?>',fmtR(r.total_entradas_brl),'ps-3 text-muted');
    h += dreRow('  <?= htmlspecialchars(__('admin.report_general.income_in_usd','Entradas em USD'), ENT_QUOTES, 'UTF-8') ?> (×'+taxa.toFixed(2)+')',fmtR(r.total_entradas_usd*taxa),'ps-3 text-muted');
    h += dreRow('  <?= htmlspecialchars(__('admin.report_general.discounts_granted','Descontos concedidos'), ENT_QUOTES, 'UTF-8') ?>',((r.total_descontos||0)>0?'-':'')+fmtR(r.total_descontos||0),(r.total_descontos||0)>0?'ps-3 text-danger':'ps-3 text-muted');
    if ((r.total_peso_kg||0) > 0) h += dreRow('  <?= htmlspecialchars(__('admin.report_general.total_weight','Peso total'), ENT_QUOTES, 'UTF-8') ?>',parseFloat(r.total_peso_kg||0).toFixed(2)+' kg','ps-3 text-muted');
    h += dreRow('<?= htmlspecialchars(__('admin.report_general.minus_total_expenses','(-) DESPESAS TOTAIS'), ENT_QUOTES, 'UTF-8') ?>',fmtR(r.total_despesas),'fw-bold text-danger border-top','fs-5');
    h += dreRow('  <?= htmlspecialchars(__('admin.report_general.expenses_in_brl','Despesas em BRL'), ENT_QUOTES, 'UTF-8') ?>',fmtR(r.total_despesas_brl),'ps-3 text-muted');
    if (r.total_despesas_usd > 0) h += dreRow('  <?= htmlspecialchars(__('admin.report_general.expenses_in_usd','Despesas em USD'), ENT_QUOTES, 'UTF-8') ?> (×'+taxa.toFixed(2)+')',fmtR(r.total_despesas_usd*taxa),'ps-3 text-muted');
    h += '<tr class="border-top" style="background:#f8fafc;"><td class="fw-bold" style="font-size:14px;"><?= htmlspecialchars(__('admin.report_general.equals_net_result','(=) RESULTADO LÍQUIDO'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end fw-bold fs-4 '+(r.resultado>=0?'text-success':'text-danger')+'">'+fmtR(r.resultado)+'</td></tr>';
    h += '<tr><td class="text-muted small"><?= htmlspecialchars(__('admin.report_general.margin','Margem'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end"><span class="badge '+(r.margem>=0?'bg-success':'bg-danger')+'">'+r.margem+'%</span></td></tr>';
    h += '</tbody></table></div></div>';

    // Resumo Mensal
    h += '<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-calendar-alt me-2 text-muted"></i><?= htmlspecialchars(__('admin.report_general.monthly_summary','Resumo Mensal'), ENT_QUOTES, 'UTF-8') ?></h6></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0" style="font-size:12px;"><thead class="table-light"><tr><th><?= htmlspecialchars(__('admin.report_general.th_month','Mês'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_income','Entradas'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_expenses','Despesas'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_result','Resultado'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_orders','Pedidos'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_exp_abbr','Desp.'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>';
    ...
    h += '<tr class="fw-bold border-top table-light"><td><?= htmlspecialchars(__('admin.report_general.accumulated','ACUMULADO'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end">'+fmtR(r.total_entradas)+'</td><td class="text-end">'+fmtR(r.total_despesas)+'</td><td class="text-end '+(r.resultado>=0?'text-success':'text-danger')+'">'+fmtR(r.resultado)+'</td><td></td><td></td></tr>';
    h += '</tbody></table></div></div></div>';

    // Gateways
    h += '<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-credit-card me-2 text-muted"></i><?= htmlspecialchars(__('admin.report_general.processing_by_gateway','Processamento por Gateway'), ENT_QUOTES, 'UTF-8') ?></h6></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0" style="font-size:12px;"><thead class="table-light"><tr><th>Gateway</th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_qty','Qtd'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_total_brl','Total (R$)'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end">✓</th><th class="text-end">⏳</th><th class="text-end">✗</th><th class="text-end">↩</th></tr></thead><tbody>';
    ...
    h += '<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3 d-flex align-items-center justify-content-between"><h6 class="fw-bold small mb-0"><i class="fas fa-shopping-cart me-2 text-muted"></i><?= htmlspecialchars(__('admin.report_general.order_income','Entradas de Pedidos'), ENT_QUOTES, 'UTF-8') ?> <span class="badge bg-secondary ms-1">'+pg.total+'</span></h6>';
    h += '<div class="d-flex align-items-center gap-2"><select id="dre-status-filter" class="form-select form-select-sm" style="width:auto;font-size:11px;" onchange="dreFilterStatus(this.value)"><option value=""><?= htmlspecialchars(__('admin.report_general.all_statuses','Todos os status'), ENT_QUOTES, 'UTF-8') ?></option>';
    ...
    h += '</select></div></div>';
    h += '<div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0" style="font-size:11px;"><thead class="table-light"><tr><th>#</th><th><?= htmlspecialchars(__('admin.report_general.th_date','Data'), ENT_QUOTES, 'UTF-8') ?></th><th><?= htmlspecialchars(__('admin.report_general.th_status','Status'), ENT_QUOTES, 'UTF-8') ?></th><th>Gateway</th><th><?= htmlspecialchars(__('admin.report_general.th_currency','Moeda'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_subtotal','Subtotal'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_fees','Taxas'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.taxes','Impostos'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end fw-bold"><?= htmlspecialchars(__('admin.report_general.th_total','Total'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody id="dre-pedidos-tbody">';
    ...
    if(!(d.entradas_detalhadas||[]).length)h+='<tr><td colspan="9" class="text-center text-muted py-3"><?= htmlspecialchars(__('admin.report_general.no_orders_period','Nenhum pedido no período'), ENT_QUOTES, 'UTF-8') ?></td></tr>';
    h += '</tbody></table></div>';
    // Botão "Ver mais" (append, não substitui)
    if (pg.total > pg.page * pg.per_page) {
        h += '<div class="text-center" style="margin-top:10px;padding-bottom:12px;" id="dre-ver-mais-wrap"><button class="btn btn-sm btn-outline-primary rounded-pill px-4" onclick="dreLoadMorePedidos()" id="dre-ver-mais-btn"><i class="fas fa-chevron-down me-1"></i><?= htmlspecialchars(__('admin.report_general.load_more_orders','Ver mais pedidos'), ENT_QUOTES, 'UTF-8') ?> ('+pg.total+' <?= htmlspecialchars(__('admin.report_general.total_word','total'), ENT_QUOTES, 'UTF-8') ?>)</button></div>';
    }
    h += '</div></div>';
    ...
    h += '<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-users me-2 text-muted"></i><?= htmlspecialchars(__('admin.report_general.operational_by_person','Operacional — Pagamentos por Pessoa/Mês'), ENT_QUOTES, 'UTF-8') ?></h6></div><div class="card-body p-0"><div class="table-responsive">';
    const opData=d.operacional||[];
    if(!opData.length){h+='<div class="text-center text-muted py-4 small"><?= htmlspecialchars(__('admin.report_general.register_expenses_favored','Cadastre despesas com favorecido para visualizar.'), ENT_QUOTES, 'UTF-8') ?></div>';}
    else{const pessoas={},mesesOp=new Set();opData.forEach(r=>{pessoas[r.pessoa]=pessoas[r.pessoa]||{};pessoas[r.pessoa][r.mes]=(pessoas[r.pessoa][r.mes]||0)+parseFloat(r.total);mesesOp.add(r.mes);});const mesesArr=[...mesesOp].sort();h+='<table class="table table-sm table-hover mb-0" style="font-size:11px;"><thead class="table-light"><tr><th><?= htmlspecialchars(__('admin.report_general.th_person','Pessoa'), ENT_QUOTES, 'UTF-8') ?></th>';mesesArr.forEach(m=>{h+='<th class="text-end">'+m+'</th>';});h+='<th class="text-end fw-bold"><?= htmlspecialchars(__('admin.report_general.th_total','Total'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>';const pArr=Object.entries(pessoas).map(([n,ms])=>({nome:n,meses:ms,total:Object.values(ms).reduce((s,v)=>s+v,0)})).sort((a,b)=>b.total-a.total);pArr.forEach(p=>{h+='<tr><td class="fw-semibold">'+p.nome+'</td>';mesesArr.forEach(m=>{h+='<td class="text-end">'+(p.meses[m]?fmtR(p.meses[m]):'-')+'</td>';});h+='<td class="text-end fw-bold">'+fmtR(p.total)+'</td></tr>';});h+='</tbody></table>';}
    h += '</div></div></div>';
    ...
    // Conciliação
    h += '<div class="card border-0 shadow-sm" style="border-top:3px solid #1e293b;"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-balance-scale me-2"></i><?= htmlspecialchars(__('admin.report_general.financial_reconciliation','Conciliação Financeira'), ENT_QUOTES, 'UTF-8') ?></h6></div><div class="card-body"><table class="table table-sm mb-0" style="font-size:13px;"><tbody>';
    h += '<tr><td><?= htmlspecialchars(__('admin.report_general.total_credits','Total de créditos (entradas)'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end fw-bold text-success">'+fmtR(d.conciliacao.total_creditos)+'</td></tr>';
    h += '<tr><td><?= htmlspecialchars(__('admin.report_general.total_debits','Total de débitos (despesas)'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end fw-bold text-danger">'+fmtR(d.conciliacao.total_debitos)+'</td></tr>';
    h += '<tr class="border-top" style="background:#f8fafc;"><td class="fw-bold"><?= htmlspecialchars(__('admin.report_general.final_balance','Saldo final'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end fw-bold fs-5 '+(d.conciliacao.saldo_final>=0?'text-success':'text-danger')+'">'+fmtR(d.conciliacao.saldo_final)+'</td></tr>';
    h += '<tr><td class="text-muted small"><?= htmlspecialchars(__('admin.report_general.entries_count','Quantidade de lançamentos'), ENT_QUOTES, 'UTF-8') ?></td><td class="text-end">'+d.conciliacao.qtd_lancamentos+'</td></tr>';
    h += '</tbody></table>';
    h += '<hr class="my-3">';
    h += '<div class="d-flex justify-content-between align-items-center mb-3"><h6 class="fw-bold mb-0"><i class="fas fa-money-bill-wave me-2"></i><?= htmlspecialchars(__('admin.report_general.cash_flow','Fluxo de Caixa'), ENT_QUOTES, 'UTF-8') ?></h6><button class="btn btn-sm btn-outline-primary" onclick="carregarFluxoCaixa(this,true)"><i class="fas fa-sync me-1"></i><?= htmlspecialchars(__('admin.report_general.refresh_apis','Atualizar APIs'), ENT_QUOTES, 'UTF-8') ?></button></div>';
    h += '<div id="conciliacao-gateways-container"><div class="text-center py-3"><i class="fas fa-spinner fa-spin text-muted"></i><div class="text-muted small mt-1"><?= htmlspecialchars(__('admin.report_general.loading_cash_flow','Carregando fluxo de caixa...'), ENT_QUOTES, 'UTF-8') ?></div></div></div>';
    h += '</div></div>';
...
function dreLoadMorePedidos() {
    ...
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i><?= htmlspecialchars(__('admin.report_general.loading','Carregando...'), ENT_QUOTES, 'UTF-8') ?>'; }
    ...
    fetch(url).then(r=>r.json()).then(d=>{
        ...
        if (wrap) {
            if (window._dreNextPage > (pg2.total_pages||1)) {
                wrap.innerHTML = '<span class="text-muted small"><?= htmlspecialchars(__('admin.report_general.all_orders_loaded','Todos os pedidos carregados'), ENT_QUOTES, 'UTF-8') ?></span>';
            } else {
                wrap.innerHTML = '<button class="btn btn-sm btn-outline-primary rounded-pill px-4" onclick="dreLoadMorePedidos()" id="dre-ver-mais-btn"><i class="fas fa-chevron-down me-1"></i><?= htmlspecialchars(__('admin.report_general.load_more_orders','Ver mais pedidos'), ENT_QUOTES, 'UTF-8') ?> ('+(pg2.total||0)+' <?= htmlspecialchars(__('admin.report_general.total_word','total'), ENT_QUOTES, 'UTF-8') ?>)</button>';
            }
        }
    }).catch(()=>{ if(btn){btn.disabled=false;btn.innerHTML='<?= htmlspecialchars(__('admin.report_general.error_try_again','Erro, tente novamente'), ENT_QUOTES, 'UTF-8') ?>';} });
}
...
function carregarFluxoCaixa(btn, force) {
    btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i><?= htmlspecialchars(__('admin.report_general.loading','Carregando...'), ENT_QUOTES, 'UTF-8') ?>';
    var container = document.getElementById('conciliacao-gateways-container');
    container.innerHTML = '<div class="text-center py-4"><i class="fas fa-spinner fa-spin fs-3 text-muted"></i><div class="text-muted small mt-2"><?= htmlspecialchars(__('admin.report_general.loading_cash_flow','Carregando fluxo de caixa...'), ENT_QUOTES, 'UTF-8') ?></div></div>';
    ...
    fetch('/admin/dre-completo/conciliacao?' + (force ? 'force=1' : '') + dateParams)
        ...
        .then(function(d){
            btn.disabled = false; btn.innerHTML = '<i class="fas fa-sync me-1"></i><?= htmlspecialchars(__('admin.report_general.refresh_apis','Atualizar APIs'), ENT_QUOTES, 'UTF-8') ?>';
            renderFluxoCaixa(d, container);
        })
        .catch(function(e){
            btn.disabled = false; btn.innerHTML = '<i class="fas fa-sync me-1"></i><?= htmlspecialchars(__('admin.report_general.refresh_apis','Atualizar APIs'), ENT_QUOTES, 'UTF-8') ?>';
            container.innerHTML = '<div class="alert alert-danger small"><?= htmlspecialchars(__('admin.report_general.error_prefix','Erro: '), ENT_QUOTES, 'UTF-8') ?>'+e.message+'</div>';
        });
}
...
function renderFluxoCaixa(d, container) {
    ...
    // Cache info
    if (d._from_cache) h += '<div class="small text-muted mb-3"><i class="fas fa-database me-1"></i><?= htmlspecialchars(__('admin.report_general.cache_of','Cache de'), ENT_QUOTES, 'UTF-8') ?> '+(d._cache_age||'?')+' <?= htmlspecialchars(__('admin.report_general.use_refresh_realtime','· Use "Atualizar APIs" para dados em tempo real'), ENT_QUOTES, 'UTF-8') ?></div>';
    ...
    h += '<div><div class="small text-muted">Câmbio Real (<?= htmlspecialchars(__('admin.report_general.products_word','Produtos'), ENT_QUOTES, 'UTF-8') ?>)</div>';
    if (cr.erro) h += '<div class="text-danger small">'+cr.erro+'</div>';
        ...
        h += '<div class="text-muted" style="font-size:10px;">'+cr.total_registros+' <?= htmlspecialchars(__('admin.report_general.records_word','registros'), ENT_QUOTES, 'UTF-8') ?> · '+cr.total_consultados+' <?= htmlspecialchars(__('admin.report_general.verified_in_api','verificados na API'), ENT_QUOTES, 'UTF-8') ?></div>';
    }
    h += '</div></div></div>';
    ...
    h += '<div><div class="small text-muted">Câmbio Real (<?= htmlspecialchars(__('admin.report_general.fees_word','Taxas'), ENT_QUOTES, 'UTF-8') ?>)</div>';
    if (crt.erro) h += '<div class="text-danger small">'+crt.erro+'</div>';
        ...
        h += '<div class="text-muted" style="font-size:10px;">'+crt.total_registros+' <?= htmlspecialchars(__('admin.report_general.records_word','registros'), ENT_QUOTES, 'UTF-8') ?> · '+crt.total_consultados+' <?= htmlspecialchars(__('admin.report_general.verified_in_api','verificados na API'), ENT_QUOTES, 'UTF-8') ?></div>';
    }
    h += '</div></div></div>';
    h += '</div>';

    // === EXTRATO / MOVIMENTAÇÃO ===
    h += '<div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white border-0 pt-3 d-flex justify-content-between align-items-center"><h6 class="fw-bold small mb-0"><i class="fas fa-list me-2"></i><?= htmlspecialchars(__('admin.report_general.movement_dre_period','Movimentação (período do DRE)'), ENT_QUOTES, 'UTF-8') ?></h6><span class="badge bg-secondary">'+(d.fluxo_caixa?d.fluxo_caixa.length:0)+' <?= htmlspecialchars(__('admin.report_general.entries_word','lançamentos'), ENT_QUOTES, 'UTF-8') ?></span></div>';
    h += '<div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0" style="font-size:12px;">';
    h += '<thead class="table-light"><tr><th><?= htmlspecialchars(__('admin.report_general.th_date','Data'), ENT_QUOTES, 'UTF-8') ?></th><th><?= htmlspecialchars(__('admin.report_general.th_description','Descrição'), ENT_QUOTES, 'UTF-8') ?></th><th>Gateway</th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_inflow','Entrada'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_outflow','Saída'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_balance','Saldo'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>';
    ...
    if (d.fluxo_caixa && d.fluxo_caixa.length) {
        ...
        h += '<tr><td colspan="6" class="text-center text-muted py-3"><?= htmlspecialchars(__('admin.report_general.no_movement_period','Nenhuma movimentação no período'), ENT_QUOTES, 'UTF-8') ?></td></tr>';
    }
    h += '</tbody></table></div></div></div>';

    // === AGENDAMENTOS FUTUROS ===
    if (d.agendamentos_futuros && d.agendamentos_futuros.length) {
        h += '<div class="card border-0 shadow-sm"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-calendar-alt me-2"></i><?= htmlspecialchars(__('admin.report_general.future_schedules','Agendamentos Futuros'), ENT_QUOTES, 'UTF-8') ?></h6></div>';
        h += '<div class="card-body p-0"><div class="table-responsive"><table class="table table-sm mb-0" style="font-size:12px;">';
        h += '<thead class="table-light"><tr><th><?= htmlspecialchars(__('admin.report_general.th_due_date','Vencimento'), ENT_QUOTES, 'UTF-8') ?></th><th><?= htmlspecialchars(__('admin.report_general.th_description','Descrição'), ENT_QUOTES, 'UTF-8') ?></th><th><?= htmlspecialchars(__('admin.report_general.th_type','Tipo'), ENT_QUOTES, 'UTF-8') ?></th><th class="text-end"><?= htmlspecialchars(__('admin.report_general.th_value','Valor'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>';
        d.agendamentos_futuros.forEach(function(ag){
            h += '<tr><td class="text-nowrap">'+(ag.vencimento||'')+'</td><td>'+ag.descricao+'</td><td><span class="badge bg-'+(ag.tipo==='entrada'?'success':'danger')+'" style="font-size:9px;">'+(ag.tipo==='entrada'?'<?= htmlspecialchars(__('admin.report_general.inflow','Entrada'), ENT_QUOTES, 'UTF-8') ?>':'<?= htmlspecialchars(__('admin.report_general.outflow','Saída'), ENT_QUOTES, 'UTF-8') ?>')+'</span></td><td class="text-end fw-semibold '+(ag.tipo==='entrada'?'text-success':'text-danger')+'">'+(ag.tipo==='entrada'?'+':'-')+fmtV(ag.valor,ag.moeda)+'</td></tr>';
        });
        h += '</tbody></table></div></div></div>';
    }
    ...
    if (divTotal > 0) {
        h += '<div class="alert alert-warning mt-4 small"><i class="fas fa-exclamation-triangle me-1"></i><strong>'+divTotal+' <?= htmlspecialchars(__('admin.report_general.divergences_found','divergência(s) encontrada(s)'), ENT_QUOTES, 'UTF-8') ?></strong> <?= htmlspecialchars(__('admin.report_general.divergences_note','entre o sistema e os gateways. Verifique os pagamentos que não batem.'), ENT_QUOTES, 'UTF-8') ?></div>';
    }
    ...
    // Total nos Gateways: usar dados REAIS das APIs
    // Stripe: saldo real da conta (disponível + pendente) — é o que está no gateway agora
    var stripeUsd = 0;
    ...
    h += '<div class="card border-0 shadow-sm mt-4 mb-4" style="border-top:3px solid #1e293b;"><div class="card-header bg-white border-0 pt-3"><h6 class="fw-bold small mb-0"><i class="fas fa-balance-scale me-2"></i><?= htmlspecialchars(__('admin.report_general.comparison_system_gateways','Comparativo: Sistema vs Gateways (BRL, taxa '), ENT_QUOTES, 'UTF-8') ?>'+taxaConv+')</h6></div><div class="card-body">';
    h += '<div class="row g-3 text-center">';
    h += '<div class="col-md-4"><div class="border rounded p-3"><div class="text-muted small"><?= htmlspecialchars(__('admin.report_general.total_income_system','Total Entradas (Sistema)'), ENT_QUOTES, 'UTF-8') ?></div><div class="fs-4 fw-bold text-success">'+fmtBRL(totalSiteBrl)+'</div><div class="text-muted" style="font-size:10px;"><?= htmlspecialchars(__('admin.report_general.paid_orders_period','Pedidos pagos no período'), ENT_QUOTES, 'UTF-8') ?></div></div></div>';
    h += '<div class="col-md-4"><div class="border rounded p-3"><div class="text-muted small"><?= htmlspecialchars(__('admin.report_general.total_in_gateways','Total nos Gateways'), ENT_QUOTES, 'UTF-8') ?></div><div class="fs-4 fw-bold text-primary">'+fmtBRL(totalGatewaysBrl)+'</div>';
    h += '<div class="text-muted" style="font-size:10px;">Stripe: '+fmtBRL(stripePedidosBrl)+' · CR Prod: '+fmtBRL(crProdBrl)+' · CR Tax: '+fmtBRL(crTaxBrl)+(outrosBrl>0?' · <?= htmlspecialchars(__('admin.report_general.others_word','Outros'), ENT_QUOTES, 'UTF-8') ?>: '+fmtBRL(outrosBrl):'')+'</div>';
    h += '</div></div>';
    var diff = totalSiteBrl - totalGatewaysBrl;
    h += '<div class="col-md-4"><div class="border rounded p-3"><div class="text-muted small"><?= htmlspecialchars(__('admin.report_general.difference','Diferença'), ENT_QUOTES, 'UTF-8') ?></div><div class="fs-4 fw-bold '+(Math.abs(diff)<500?'text-success':'text-danger')+'">'+fmtBRL(diff)+'</div>'+(Math.abs(diff)<500?'<div class="text-success small"><i class="fas fa-check-circle"></i> <?= htmlspecialchars(__('admin.report_general.reconciled','Conciliado'), ENT_QUOTES, 'UTF-8') ?></div>':'<div class="text-danger small"><i class="fas fa-exclamation-triangle"></i> <?= htmlspecialchars(__('admin.report_general.check_word','Verificar'), ENT_QUOTES, 'UTF-8') ?></div>')+'</div></div>';
    h += '</div>';
    // Detalhamento
    h += '<div class="mt-3 small text-muted border-top pt-2">';
    h += '<strong><?= htmlspecialchars(__('admin.report_general.composition','Composição:'), ENT_QUOTES, 'UTF-8') ?></strong> Stripe '+fmtBRL(stripePedidosBrl);
    h += ' · CR <?= htmlspecialchars(__('admin.report_general.products_word','Produtos'), ENT_QUOTES, 'UTF-8') ?> '+fmtBRL(crProdBrl)+' · CR <?= htmlspecialchars(__('admin.report_general.fees_word','Taxas'), ENT_QUOTES, 'UTF-8') ?> '+fmtBRL(crTaxBrl);
    if (outrosBrl > 0) h += ' · <?= htmlspecialchars(__('admin.report_general.others_word','Outros'), ENT_QUOTES, 'UTF-8') ?> '+fmtBRL(outrosBrl);
    h += '<br><i class="fas fa-info-circle me-1"></i><?= htmlspecialchars(__('admin.report_general.source_note','Fonte: tabela de pedidos pagos agrupados por gateway (mesma do "Total Entradas")'), ENT_QUOTES, 'UTF-8') ?>';
    // Mostrar gateways raw para debug
    if (d.totais_por_gateway && d.totais_por_gateway.length) {
        h += '<br><span style="font-size:10px;"><?= htmlspecialchars(__('admin.report_general.gateways_found','Gateways encontrados: '), ENT_QUOTES, 'UTF-8') ?>';
        d.totais_por_gateway.forEach(function(g){ h += g.gateway+'('+g.moeda+':'+parseFloat(g.total).toFixed(0)+' x'+g.qtd+') '; });
        h += '</span>';
    }
    h += '</div>';
    h += '</div></div>';
    ...
    gateways.forEach(function(gw){
        ...
        if (movs.length === 0) h += '<tr><td class="text-center text-muted py-3"><?= htmlspecialchars(__('admin.report_general.no_movement','Sem movimentação'), ENT_QUOTES, 'UTF-8') ?></td></tr>';
        ...
        h += '</tbody></table></div>';
        h += '<div class="card-footer bg-white border-top text-end"><span class="fw-bold small"><?= htmlspecialchars(__('admin.report_general.total_label','Total: '), ENT_QUOTES, 'UTF-8') ?><span class="'+(totalBrl>=0?'text-success':'text-danger')+'">'+fmtBRL(totalBrl)+'</span></span></div>';
        h += '</div></div>';
    });
    h += '</div>';
