    ...
    <div class="card-body py-3">
        <form class="row g-2" method="GET" action="/admin/promocoes-auditoria" id="promoAuditoriaForm">
            ...
            <div class="col-6 col-md-5">
                <input type="text" class="form-control form-control-sm" name="busca" value="<?= htmlspecialchars($filtroBusca) ?>" placeholder="Buscar produto..." oninput="clearTimeout(this._t);this._t=setTimeout(()=>this.form.submit(),400)">
            </div>
            <div class="col-6 col-md-2">
                <a class="btn btn-sm btn-outline-secondary w-100" href="/admin/promocoes-auditoria">Limpar</a>
            </div>
        </form>
    </div>
</div>
...
<!-- Promoções Atuais -->
<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white fw-semibold">Promoções nos Produtos (estado atual)</div>
    <div class="card-body">
                ...
                <table class="table table-sm table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="<?= $expirada ? 'table-warning' : '' ?>">
                            ...
                            <td class="small <?= $expirada ? 'text-danger' : 'text-muted' ?>"><?= $expires ? date('d/m/Y H:i', strtotime($expires)) : 'Sem prazo' ?></td>
                            <td><?= $expirada ? '<span class="badge bg-warning text-dark">Expirada</span>' : ($promo > 0 ? '<span class="badge bg-success">Ativa</span>' : '') ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="border-bottom py-2">
                    <div class="d-flex justify-content-between align-items-start">
                        <div style="min-width:0;flex:1;">
                            ...
                            <div class="text-muted small mt-1"><?= $expires ? ($expirada ? '<span class="text-danger">Expirou ' : 'Expira ') . date('d/m/Y', strtotime($expires)) . ($expirada ? '</span>' : '') : 'Sem prazo' ?></div>
                        </div>
                        <div class="ms-2">
                            <?= $expirada ? '<span class="badge bg-warning text-dark">Expirada</span>' : ($promo > 0 ? '<span class="badge bg-success">Ativa</span>' : '') ?>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
    ...
    <div class="card-body">
            ...
            <table class="table table-sm table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="/admin/produtos/editar/<?= (int) $h['produto_id'] ?>" class="text-decoration-none"><?= htmlspecialchars((string) ($h['produto_nome'] ?? 'Produto #' . $h['produto_id'])) ?></a></td>
        ...
        <div class="d-md-none">
            ...
            <div class="border-bottom py-2">
                ...
                <div class="fw-semibold small mt-1" style="word-break:break-word;"><?= htmlspecialchars((string) ($h['produto_nome'] ?? 'Produto #' . $h['produto_id'])) ?></div>
