...
function renderDocPage(){var el=document.getElementById('doc-tab-lista');var totalPages=Math.ceil(docAllFaturas.length/docPerPage);var start=(docCurrentPage-1)*docPerPage;var end=Math.min(start+docPerPage,docAllFaturas.length);var items=docAllFaturas.slice(start,end);var html='';items.forEach(function(fat,li){var gi=start+li;var cn38=fat.cn38_code||'-';var dns=Array.isArray(fat.dispatch_numbers)?fat.dispatch_numbers.map(function(d){return String(d);}):[];var isFirst=gi===0;var badge=isFirst?' <span class="badge bg-info text-dark" style="font-size:.6rem;">Ultima</span>':'';var status=fat.departure_id?'<span class="badge bg-success" style="font-size:.65rem;">Embarcado</span>':'<span class="badge bg-warning text-dark" style="font-size:.65rem;">Aguardando</span>';var fatCnts=docAllContainers.filter(function(c){return dns.indexOf(String(c.dispatch_number))!==-1;});var docsCount=(fat.wp_post_id?1:0)+fatCnts.filter(function(c){return c.wp_post_id;}).length;html+='<div class="border rounded mb-2" id="dti-'+gi+'" style="overflow:hidden;">';html+='<div class="d-flex align-items-center justify-content-between px-3 py-2" style="cursor:pointer;background:#fff;" onclick="toggleDti('+gi+')">';html+='<div class="d-flex align-items-center gap-2 flex-wrap" style="min-width:0;flex:1;"><i class="fas fa-chevron-right dti-chev-'+gi+'" style="font-size:.6rem;color:#94a3b8;transition:transform .2s;'+(gi===start?'transform:rotate(90deg);':'')+'"></i><strong class="small">'+escHtmlDoc(cn38)+'</strong> '+badge+' '+status+' <span class="text-muted" style="font-size:.75rem;">Rem: '+escHtmlDoc(dns.join(', ')||'-')+'</span></div>';html+='<div class="d-flex align-items-center gap-2 flex-shrink-0"><span class="badge bg-light text-dark border" style="font-size:.65rem;">'+docsCount+' PDF'+(docsCount!==1?'s':'')+'</span><button class="btn btn-sm btn-primary py-0 px-2" onclick="event.stopPropagation();baixarDocsFaturaTab('+gi+')" style="font-size:.72rem;"><i class="fas fa-download me-1"></i>Baixar</button></div>';html+='</div>';html+='<div id="dti-body-'+gi+'" style="'+(gi===start?'':'display:none;')+'padding:8px 16px 12px;background:#f8fafc;border-top:1px solid #e2e8f0;"><div class="row"><div class="col-md-4 mb-1"><div style="font-size:.68rem;text-transform:uppercase;font-weight:700;color:#64748b;margin-bottom:4px;">Fatura</div>';if(fat.wp_post_id){html+='<a href="'+BASE+'/pdf/fatura/'+fat.wp_post_id+'" target="_blank" class="dti-pdf-'+gi+'" data-url="'+BASE+'/pdf/fatura/'+fat.wp_post_id+'" data-name="fatura_'+escHtmlDoc(cn38)+'" style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border:1px solid #e2e8f0;border-radius:5px;font-size:.78rem;text-decoration:none;color:#1e293b;background:#fff;"><i class="fas fa-file-pdf text-danger"></i>'+escHtmlDoc(cn38)+'</a>';}else{html+='<span class="text-muted small">-</span>';}html+='</div><div class="col-md-8 mb-1"><div style="font-size:.68rem;text-transform:uppercase;font-weight:700;color:#64748b;margin-bottom:4px;">Containers</div><div class="d-flex flex-wrap gap-1">';if(fatCnts.length>0){fatCnts.forEach(function(c){var tk=Array.isArray(c.tracking_codes)?c.tracking_codes.length:0;if(c.wp_post_id){html+='<a href="'+BASE+'/pdf/container/'+c.wp_post_id+'" target="_blank" class="dti-pdf-'+gi+'" data-url="'+BASE+'/pdf/container/'+c.wp_post_id+'" data-name="container_'+c.dispatch_number+'" style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border:1px solid #e2e8f0;border-radius:5px;font-size:.78rem;text-decoration:none;color:#1e293b;background:#fff;"><i class="fas fa-file-pdf text-danger"></i>Rem '+c.dispatch_number+' <span class="text-muted" style="font-size:.65rem;">('+tk+')</span></a>';}else{html+='<span class="small text-muted border rounded px-2 py-1" style="font-size:.75rem;">Rem '+c.dispatch_number+'</span>';}});}else{html+='<span class="text-muted small">Nenhum</span>';}html+='</div></div></div></div>';html+='</div>';});el.innerHTML=html;renderDocPagination(totalPages);}
function renderDocPagination(tp){var pag=document.getElementById('doc-tab-pagination');if(tp<=1){pag.innerHTML='';return;}var html='<div class="d-flex align-items-center justify-content-center gap-1 mt-3">';html+='<button class="btn btn-sm btn-outline-secondary py-0 px-2" onclick="docGoPage('+(docCurrentPage-1)+')" '+(docCurrentPage===1?'disabled':'')+' style="font-size:.75rem;"><i class="fas fa-chevron-left"></i></button>';for(var i=1;i<=tp;i++){if(tp<=7||i<=2||i>=tp-1||Math.abs(i-docCurrentPage)<=1){html+='<button class="btn btn-sm '+(i===docCurrentPage?'btn-primary':'btn-outline-secondary')+' py-0 px-2" onclick="docGoPage('+i+')" style="font-size:.75rem;">'+i+'</button>';}else if(i===3&&docCurrentPage>4){html+='<span class="small text-muted">...</span>';}else if(i===tp-2&&docCurrentPage<tp-3){html+='<span class="small text-muted">...</span>';}}html+='<button class="btn btn-sm btn-outline-secondary py-0 px-2" onclick="docGoPage('+(docCurrentPage+1)+')" '+(docCurrentPage===tp?'disabled':'')+' style="font-size:.75rem;"><i class="fas fa-chevron-right"></i></button>';html+='<span class="text-muted small ms-2">'+docAllFaturas.length+' fatura(s)</span></div>';pag.innerHTML=html;}
