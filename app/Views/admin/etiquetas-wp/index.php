...
<style>
    ...
    .cnt-row .cnt-chevron{display:inline-block;transition:transform .2s;font-size:.7rem;color:#6c757d}
    .cnt-row.expanded .cnt-chevron{transform:rotate(90deg)}
...
async function loadContainerDetail(idx){const c=window._containersData[idx];if(!c)return;const el=document.getElementById('cnt-detail-content-'+idx);const tks=Array.isArray(c.tracking_codes)?c.tracking_codes:[];let html='<div class="cnt-info-grid">';html+=cntInfoItem('País Origem',c.origin_country||'US');html+=cntInfoItem('Operador Origem',c.origin_operator||'BRAZ');html+=cntInfoItem('Operador Destino',c.destination_operator||'-');html+=cntInfoItem('Categoria Postal',c.postal_category||'-');html+=cntInfoItem('Subclasse',c.service_subclass||'-');html+=cntInfoItem('Tipo Unidade',formatCntUnitType(c.unit_type));html+=cntInfoItem('AWB',c.awb||'-');html+=cntInfoItem('Grupo Triagem',c.triage_group||'-');html+=cntInfoItem('Total Pacotes',tks.length);html+='</div>';if(tks.length>0){html+='<h6 class="mt-2 mb-1" style="font-size:.85rem"><i class="fas fa-box me-1"></i>Pacotes ('+tks.length+')</h6>';html+='<div class="table-responsive"><table class="table table-sm table-bordered cnt-pkg-table mb-0"><thead><tr><th>#</th><th>Tracking Code</th></tr></thead><tbody>';tks.forEach((tk,i)=>{html+='<tr><td>'+(i+1)+'</td><td><code>'+escHtmlCnt(tk)+'</code></td></tr>';});html+='</tbody></table></div>';}else{html+='<p class="text-muted small mb-0"><i class="fas fa-info-circle me-1"></i>Nenhum pacote vinculado.</p>';}el.innerHTML=html;_cntDetailsLoaded[idx]=true;}
