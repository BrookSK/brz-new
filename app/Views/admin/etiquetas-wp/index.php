            ...
            <div class="card-body">
                <form id="form-container" onsubmit="criarContainer(event)">
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-md-4">
                            <select class="form-select form-select-sm" id="cnt-selecionar-mala" onchange="selecionarPorMala()">
                                <option value="">Selecionar por Mala...</option>
                                <option value="all">Todos os pacotes</option>
                                <option value="mala-1">Mala 1 - São Paulo/SP</option>
                                <option value="mala-2">Mala 2 - Valinhos/SP</option>
                                <option value="mala-3">Mala 3 - Rio de Janeiro/RJ</option>
                                <option value="mala-4">Mala 4 - Curitiba/PR</option>
                                <option value="mala-5">Mala 5 - Curitiba/PR</option>
                            </select>
                        </div>
                        <div class="col-md-4"><textarea class="form-control form-control-sm" id="cnt-paste-trackings" rows="2" placeholder="Cole tracking codes (um por linha)"></textarea></div>
                        <div class="col-md-4 d-flex align-items-end gap-2"><button type="button" class="btn btn-sm btn-outline-primary" onclick="validarESelecionar()"><i class="fas fa-check-double me-1"></i>Validar e Selecionar</button></div>
                    </div>
...
// CONTAINERS
async function carregarPacotesParaContainer(){const el=document.getElementById('cnt-pacotes-lista');try{const r=await fetch(BASE+'/listar-pacotes?without_container=1&per_page=200');const d=await r.json();if(d.success&&d.data&&d.data.length>0){const available=d.data.filter(p=>p.package_status!=='cancelado');if(available.length>0){let h='<div class="d-flex justify-content-between mb-1"><small class="text-muted">'+available.length+' pacote(s)</small><a href="#" onclick="document.querySelectorAll(\'.chk-cnt-pacote\').forEach(e=>e.checked=true);updateCntCount();return false;" class="small">Todos</a></div>';available.forEach(p=>{const mala=p.triage_group||'';const malaLabel=mala?(' <span class="badge bg-secondary" style="font-size:.6rem;">Mala '+mala+'</span>'):'';h+='<div class="form-check"><input class="form-check-input chk-cnt-pacote" type="checkbox" value="'+p.tracking_code+'" data-mala="'+mala+'"><label class="form-check-label small"><code>'+p.tracking_code+'</code> — '+(p.order_id||'')+malaLabel+'</label></div>';});el.innerHTML=h;}else{el.innerHTML='<span class="small text-muted">Nenhum pacote disponível (todos cancelados ou já em container)</span>';}}else{el.innerHTML='<span class="small text-muted">Nenhum pacote disponível</span>';}}catch(e){el.innerHTML='<span class="text-danger small">'+e.message+'</span>';}autoPreencherRemessa();}
function selecionarPorMala(){const val=document.getElementById('cnt-selecionar-mala').value;if(!val)return;const checkboxes=document.querySelectorAll('.chk-cnt-pacote');if(val==='all'){checkboxes.forEach(cb=>{cb.checked=true;});}else{const malaNum=val.replace('mala-','');checkboxes.forEach(cb=>{const cbMala=cb.getAttribute('data-mala')||'';if(cbMala===malaNum){cb.checked=true;}});if(document.querySelectorAll('.chk-cnt-pacote:checked').length===0){checkboxes.forEach(cb=>{cb.checked=true;});alert('Nenhum pacote com Mala '+malaNum+' encontrado. Todos foram selecionados.');}}updateCntCount();const triageSelect=document.getElementById('cnt-triage');if(val.startsWith('mala-')&&triageSelect){triageSelect.value=val.replace('mala-','');}}
