...
// Notifica TODOS os clientes dos pedidos de um embarque (resolve embarque -> faturas -> trackings -> pedidos).
async function notificarEmbarque(wpPostId,btn){if(!confirm(<?= json_encode(__('admin.labels_wp.confirm_notify_shipment','Enviar notificação de rastreio (e-mail + WhatsApp) para TODOS os clientes deste embarque?')) ?>))return;var orig=btn?btn.innerHTML:'';if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>';}try{const r=await fetch(BASE+'/notificar-embarque',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({wp_post_id:wpPostId})});const d=await r.json();if(d.success){alert((<?= json_encode(__('admin.labels_wp.notify_result','Notificações enviadas: {ok}. Falhas: {fail}.')) ?>).replace('{ok}', d.enviadas||0).replace('{fail}', d.falhas||0));}else{alert((<?= json_encode(__('admin.labels_wp.error_prefix','Erro:')) ?>)+' '+(d.error||(<?= json_encode(__('admin.labels_wp.notify_failed','Falha ao enviar notificações.')) ?>)));}}catch(e){alert(e.message);}finally{if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>';}}}
...
// Notifica TODOS os clientes dos pedidos de uma fatura (resolve fatura -> containers -> trackings -> pedidos).
async function notificarFatura(billWpPostId,btn){if(!billWpPostId){alert(<?= json_encode(__('admin.labels_wp.invalid_wp_post_id','wp_post_id inválido')) ?>);return;}if(!confirm(<?= json_encode(__('admin.labels_wp.confirm_notify_invoice','Enviar notificação de rastreio (e-mail + WhatsApp) para TODOS os clientes desta fatura?')) ?>))return;var orig=btn?btn.innerHTML:'';if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>';}try{const r=await fetch(BASE+'/notificar-fatura',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bill_wp_post_id:billWpPostId})});const d=await r.json();if(d.success){alert((<?= json_encode(__('admin.labels_wp.notify_result','Notificações enviadas: {ok}. Falhas: {fail}.')) ?>).replace('{ok}', d.enviadas||0).replace('{fail}', d.falhas||0));}else{alert((<?= json_encode(__('admin.labels_wp.error_prefix','Erro:')) ?>)+' '+(d.error||(<?= json_encode(__('admin.labels_wp.notify_failed','Falha ao enviar notificações.')) ?>)));}}catch(e){alert(e.message);}finally{if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane me-1"></i>'+(<?= json_encode(__('admin.labels_wp.notify_customers','Notificar clientes')) ?>);}}}
