        ...
        <div class="card ewp-card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center py-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    <select class="form-select form-select-sm" id="mala-geracaoMassa" style="width:auto;min-width:140px;display:none;"><option value="">Sem mala</option></select>
...
<?php endforeach;endif;?>
        ...
        <div class="card ewp-card">
            <div class="card-header d-flex justify-content-between align-items-center py-2 flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <select class="form-select form-select-sm" id="mala-mover-select" style="width:auto;min-width:160px;display:none;"><option value="">Selecione a mala...</option></select>
                    <button class="btn btn-sm btn-primary" id="btnMoverParaMala" style="display:none;" onclick="moverParaMala()"><i class="fas fa-suitcase me-1"></i><span id="btnMoverParaMalaText">Mover para mala</span></button>
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-md-3">
                            <select class="form-select form-select-sm" id="cnt-selecionar-mala" onchange="selecionarPacotesPorMala()">
                                <option value="">Selecionar por Mala...</option>
                            </select>
                        </div>
                        <div class="col-md-5"><textarea class="form-control form-control-sm" id="cnt-paste-trackings" rows="2" placeholder="Cole tracking codes (um por linha)"></textarea></div>
                        <div class="col-md-4 d-flex align-items-end"><button type="button" class="btn btn-sm btn-outline-primary" onclick="validarESelecionar()"><i class="fas fa-check-double me-1"></i>Validar e Selecionar</button></div>
                    </div>
    ...
    <div class="ewp-panel" id="panel-documentacao" style="display:none;">
        <div class="card ewp-card">
            <div class="card-header d-flex justify-content-between align-items-center py-2">
                <strong class="small">Documentação por Fatura</strong>
                <button class="btn btn-sm btn-outline-primary" onclick="carregarDocumentacaoTab()"><i class="fas fa-sync me-1"></i>Atualizar</button>
            </div>
            <div class="card-body">
                <p class="text-muted small mb-3">Clique em uma fatura para expandir e ver os documentos. Use "Baixar" para baixar todos os PDFs de uma vez.</p>
                <div id="doc-tab-loading" class="text-center py-3" style="display:none;"><i class="fas fa-spinner fa-spin me-1"></i> Carregando...</div>
...
async function buscarNoWordPress(termo) {
    ...
    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-3"><i class="fas fa-spinner fa-spin me-1"></i> Buscando...</td></tr>';
    try {
        ...
        if (d.success && d.data && d.data.length > 0) {
            ...
            tbody.innerHTML = '<tr><td colspan="7" class="ewp-empty"><i class="fas fa-search"></i> Nenhum pacote encontrado para "' + termo + '"</td></tr>';
            document.getElementById('pacotes-pagination').style.display = 'none';
        }
    } catch(e) {
        ...
        if (d.success && d.data && d.data.length > 0) {
            ...
            if (semMala.length > 0) {
                if (Object.keys(byMala).length > 0) {
                    html += '<tr><td colspan="7" style="background:#f8fafc;padding:6px 12px;border-left:3px solid #94a3b8;"><strong class="small text-muted"><i class="fas fa-inbox me-1"></i>Sem mala</strong> <span class="badge bg-secondary ms-1">' + semMala.length + '</span></td></tr>';
                }
                semMala.forEach(function(p) { html += renderPacoteRow(p); });
            }
            tbody.innerHTML = html;
        } else {
            tbody.innerHTML = '<tr><td colspan="7" class="ewp-empty"><i class="fas fa-inbox"></i>Nenhum pacote sem container</td></tr>';
            pacotesTotal = 0;
            pacotesPages = 1;
        }
...
async function carregarMalasMoverSelect(){try{const r=await fetch(BASE+'/listar-malas');const d=await r.json();const sel=document.getElementById('mala-mover-select');if(!sel)return;const atual=sel.value;sel.innerHTML='<option value="">Selecione a mala...</option>';if(d.success&&d.data){d.data.forEach(function(m){sel.innerHTML+='<option value="'+m.id+'">'+escHtmlCnt(m.nome)+' ('+m.pacotes_count+' pct)</option>';});}sel.value=atual;}catch(e){}}
// Mostra/esconde o select+botão conforme houver etiquetas selecionadas.
function updateMoverMala(){const n=document.querySelectorAll('.chk-pacote-dl:checked').length;const sel=document.getElementById('mala-mover-select');const btn=document.getElementById('btnMoverParaMala');const txt=document.getElementById('btnMoverParaMalaText');if(sel)sel.style.display=n>0?'':'none';if(btn)btn.style.display=n>0?'':'none';if(txt)txt.textContent=n>1?('Mover '+n+' para mala'):'Mover para mala';}
// Atribui as etiquetas selecionadas à mala escolhida.
async function moverParaMala(){const checks=[...document.querySelectorAll('.chk-pacote-dl:checked')];if(!checks.length){alert('Selecione pelo menos 1 etiqueta.');return;}const sel=document.getElementById('mala-mover-select');const malaId=sel?parseInt(sel.value):0;if(!malaId){alert('Selecione a mala de destino.');return;}const trackings=[];const pedidoIds=[];checks.forEach(function(cb){const tc=cb.getAttribute('data-tracking')||'';if(tc){trackings.push(tc);pedidoIds.push(parseInt(cb.getAttribute('data-pedido-id'))||0);}});if(!trackings.length){alert('Etiquetas selecionadas sem tracking válido.');return;}const btn=document.getElementById('btnMoverParaMala');if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-1"></i>Movendo...';}try{const r=await fetch(BASE+'/atribuir-mala',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mala_id:malaId,tracking_codes:trackings,pedido_ids:pedidoIds})});const d=await r.json();if(d.success){alert((d.added||trackings.length)+' etiqueta(s) movida(s) para a mala com sucesso!');carregarPacotes();carregarMalas();carregarMalasMoverSelect();carregarMalasSelect();}else{alert('Erro: '+(d.error||'Falha ao mover'));}}catch(e){alert('Erro: '+e.message);}if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-suitcase me-1"></i><span id="btnMoverParaMalaText">Mover para mala</span>';}updateMoverMala();}
...
async function carregarDocumentacaoTab(){var el=document.getElementById('doc-tab-lista');var loading=document.getElementById('doc-tab-loading');el.innerHTML='';document.getElementById('doc-tab-pagination').innerHTML='';loading.style.display='block';try{var r=await Promise.all([fetch(BASE+'/listar-containers?per_page=500'),fetch(BASE+'/listar-faturas?per_page=500')]);var dCnt=await r[0].json();var dFat=await r[1].json();loading.style.display='none';docAllContainers=(dCnt.success&&dCnt.data)?dCnt.data:[];docAllFaturas=(dFat.success&&dFat.data)?dFat.data:[];if(!docAllFaturas.length){el.innerHTML='<div class="text-muted text-center py-4">Nenhuma fatura encontrada.</div>';return;}docCurrentPage=1;renderDocPage();}catch(e){loading.style.display='none';el.innerHTML='<div class="alert alert-danger small">Erro: '+e.message+'</div>';}}
function renderDocPage(){var el=document.getElementById('doc-tab-lista');var totalPages=Math.ceil(docAllFaturas.length/docPerPage);var start=(docCurrentPage-1)*docPerPage;var end=Math.min(start+docPerPage,docAllFaturas.length);var items=docAllFaturas.slice(start,end);var html='';items.forEach(function(fat,li){var gi=start+li;var cn38=fat.cn38_code||'-';var dns=Array.isArray(fat.dispatch_numbers)?fat.dispatch_numbers.map(function(d){return String(d);}):[];var isFirst=gi===0;var badge=isFirst?' <span class="badge bg-info text-dark">Última</span>':'';var status=fat.departure_id?'<span class="badge bg-success">Embarcado</span>':'<span class="badge bg-warning text-dark">Aguardando</span>';var fatCnts=docAllContainers.filter(function(c){return dns.indexOf(String(c.dispatch_number))!==-1;});var docsCount=(fat.wp_post_id?1:0)+fatCnts.filter(function(c){return c.wp_post_id;}).length;html+='<div class="border rounded mb-3 shadow-sm" id="dti-'+gi+'" style="overflow:hidden;">';html+='<div class="d-flex align-items-center justify-content-between px-3 py-3" style="cursor:pointer;background:#fff;" onclick="toggleDti('+gi+')">';html+='<div class="d-flex align-items-center gap-2 flex-wrap" style="min-width:0;flex:1;"><i class="fas fa-chevron-right dti-chev-'+gi+'" style="font-size:.75rem;color:#64748b;transition:transform .2s;'+(gi===start?'transform:rotate(90deg);':'')+'"></i> <span style="font-size:.95rem;font-weight:600;">'+escHtmlDoc(cn38)+'</span> '+badge+' '+status+' <span class="text-muted" style="font-size:.85rem;">Remessas: '+escHtmlDoc(dns.join(', ')||'-')+'</span></div>';html+='<div class="d-flex align-items-center gap-2 flex-shrink-0"><span class="badge bg-light text-dark border" style="font-size:.8rem;padding:5px 10px;">'+docsCount+' PDF'+(docsCount!==1?'s':'')+'</span><button class="btn btn-primary" onclick="event.stopPropagation();baixarDocsFaturaTab('+gi+')" style="font-size:.85rem;padding:6px 14px;"><i class="fas fa-download me-1"></i>Baixar</button></div>';html+='</div>';html+='<div id="dti-body-'+gi+'" style="'+(gi===start?'':'display:none;')+'padding:16px 20px;background:#f8fafc;border-top:1px solid #e2e8f0;"><div class="row"><div class="col-md-4 mb-2"><div style="font-size:.8rem;text-transform:uppercase;font-weight:700;color:#475569;margin-bottom:8px;"><i class="fas fa-file-invoice me-1 text-danger"></i>Fatura</div>';if(fat.wp_post_id){html+='<a href="'+BASE+'/pdf/fatura/'+fat.wp_post_id+'" target="_blank" class="btn btn-outline-danger dti-pdf-'+gi+'" data-url="'+BASE+'/pdf/fatura/'+fat.wp_post_id+'" data-name="fatura_'+escHtmlDoc(cn38)+'" style="font-size:.85rem;padding:8px 16px;"><i class="fas fa-file-pdf me-2"></i>'+escHtmlDoc(cn38)+'</a>';}else{html+='<span class="text-muted">-</span>';}html+='</div><div class="col-md-8 mb-2"><div style="font-size:.8rem;text-transform:uppercase;font-weight:700;color:#475569;margin-bottom:8px;"><i class="fas fa-box me-1 text-primary"></i>Containers</div><div class="d-flex flex-wrap gap-2">';if(fatCnts.length>0){fatCnts.forEach(function(c){var tk=Array.isArray(c.tracking_codes)?c.tracking_codes.length:0;if(c.wp_post_id){html+='<a href="'+BASE+'/pdf/container/'+c.wp_post_id+'" target="_blank" class="btn btn-outline-primary dti-pdf-'+gi+'" data-url="'+BASE+'/pdf/container/'+c.wp_post_id+'" data-name="container_'+c.dispatch_number+'" style="font-size:.85rem;padding:8px 16px;"><i class="fas fa-file-pdf me-2"></i>Rem '+c.dispatch_number+' <span class="text-muted ms-1">('+tk+' pct)</span></a>';}else{html+='<span class="btn btn-outline-secondary disabled" style="font-size:.85rem;padding:8px 16px;">Rem '+c.dispatch_number+'</span>';}});}else{html+='<span class="text-muted">Nenhum container vinculado</span>';}html+='</div></div></div></div>';html+='</div>';});el.innerHTML=html;renderDocPagination(totalPages);}
function renderDocPagination(tp){var pag=document.getElementById('doc-tab-pagination');if(tp<=1){pag.innerHTML='';return;}var html='<div class="d-flex align-items-center justify-content-center gap-2 mt-4">';html+='<button class="btn btn-outline-secondary" onclick="docGoPage('+(docCurrentPage-1)+')" '+(docCurrentPage===1?'disabled':'')+' style="padding:6px 12px;"><i class="fas fa-chevron-left"></i></button>';for(var i=1;i<=tp;i++){if(tp<=7||i<=2||i>=tp-1||Math.abs(i-docCurrentPage)<=1){html+='<button class="btn '+(i===docCurrentPage?'btn-primary':'btn-outline-secondary')+'" onclick="docGoPage('+i+')" style="padding:6px 12px;min-width:38px;">'+i+'</button>';}else if(i===3&&docCurrentPage>4){html+='<span class="text-muted px-1">...</span>';}else if(i===tp-2&&docCurrentPage<tp-3){html+='<span class="text-muted px-1">...</span>';}}html+='<button class="btn btn-outline-secondary" onclick="docGoPage('+(docCurrentPage+1)+')" '+(docCurrentPage===tp?'disabled':'')+' style="padding:6px 12px;"><i class="fas fa-chevron-right"></i></button>';html+='<span class="text-muted ms-3">'+docAllFaturas.length+' fatura(s)</span></div>';pag.innerHTML=html;}
...
async function baixarDocsFaturaTab(gi){var links=document.querySelectorAll('.dti-pdf-'+gi);if(!links.length){alert('Nenhum PDF.');return;}var item=document.getElementById('dti-'+gi);var btn=item?item.querySelector('.btn-primary'):null;if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>';}var dl=0;for(var i=0;i<links.length;i++){var url=links[i].getAttribute('data-url');var name=links[i].getAttribute('data-name')||'doc';if(!url)continue;try{var resp=await fetch(url);if(!resp.ok)continue;var blob=await resp.blob();var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name+'.pdf';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(a.href);dl++;await new Promise(function(r){setTimeout(r,400);});}catch(e){}}if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-download me-1"></i>Baixar';}if(dl>0)alert(dl+' PDF(s) baixado(s)!');}
// REGERAR ETIQUETA
