...
<?php endforeach;endif;?>
        ...
        <div class="card ewp-card">
            ...
            <div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0 align-middle">
                <thead class="table-light"><tr><th>Pedido</th><th>Tracking</th><th class="d-none d-md-table-cell">Peso</th><th>PDF</th></tr></thead>
                <tbody id="pacotes-body"><tr><td colspan="4" class="text-center text-muted py-3"><i class="fas fa-spinner fa-spin me-1"></i></td></tr></tbody>
            </table></div></div></div>
    </div>
    ...
    <div class="ewp-panel" id="panel-containers" style="display:none;">
            ...
            <div class="card-body">
                <form id="form-container" onsubmit="criarContainer(event)">
                    <div class="row g-2 mb-3">
                        ...
                        <div class="col-6 col-md-2"><label class="form-label small">N° AWB</label><input type="text" class="form-control form-control-sm" id="cnt-awb" placeholder="(opcional)"></div>
        ...
        <div class="card ewp-card">
            ...
            <div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover mb-0 align-middle">
                <thead class="table-light"><tr><th>Remessa</th><th>Unit Code</th><th>Pacotes</th><th>Status</th><th class="d-none d-md-table-cell">Data</th><th>PDF</th><th>Ações</th></tr></thead>
                <tbody id="containers-body"><tr><td colspan="7" class="text-center text-muted py-3"><i class="fas fa-spinner fa-spin me-1"></i></td></tr></tbody>
            </table></div></div>
        </div>
    </div>
...
// PACOTES
async function carregarPacotes(){const tbody=document.getElementById('pacotes-body');try{const r=await fetch(BASE+'/listar-pacotes?without_container=1');const d=await r.json();tbody.innerHTML='';if(d.success&&d.data&&d.data.length>0){d.data.forEach(p=>{tbody.innerHTML+='<tr><td>'+(p.order_id||'-')+'</td><td><code class="small">'+(p.tracking_code||'-')+'</code></td><td class="d-none d-md-table-cell">'+(p.total_weight?(p.total_weight/1000).toFixed(1)+'kg':'-')+'</td><td>'+(p.wp_post_id?'<a href="'+BASE+'/pdf/pacote/'+p.wp_post_id+'" target="_blank" class="btn btn-xs btn-outline-danger"><i class="fas fa-file-pdf"></i></a>':'')+'</td></tr>';});}else{tbody.innerHTML='<tr><td colspan="4" class="ewp-empty"><i class="fas fa-inbox"></i>Nenhum pacote sem container</td></tr>';}}catch(e){tbody.innerHTML='<tr><td colspan="4" class="text-danger">'+e.message+'</td></tr>';}}
