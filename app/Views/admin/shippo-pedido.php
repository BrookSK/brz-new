...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="page-title">
            Shippo - Pedido #<?= str_pad((string) $pid, 6, '0', STR_PAD_LEFT) ?>
        </h1>
        <div class="d-flex gap-2">
            <a class="btn btn-sm btn-outline-secondary" href="/admin/shippo"><i class="fas fa-arrow-left me-1"></i>Voltar</a>
            <a class="btn btn-sm btn-outline-primary" href="/admin/pedidos/detalhes/<?= $pid ?>" target="_blank"><i class="fas fa-external-link-alt me-1"></i>Ver Pedido</a>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-6 mb-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header"><strong>Dados do Destinatário</strong></div>
                <div class="card-body">
                    <table class="table table-sm mb-0">
                        <tr><th style="width:130px;">Cliente</th><td><?= htmlspecialchars($clienteNome) ?></td></tr>
                        <tr><th>Email</th><td><?= htmlspecialchars($clienteEmail) ?></td></tr>
                        <tr><th>Telefone</th><td><?= htmlspecialchars($clienteTelefone) ?></td></tr>
                        <tr><th>Endereço</th><td><?= htmlspecialchars($endereco) ?><?= $complemento ? ', ' . htmlspecialchars($complemento) : '' ?></td></tr>
                        <tr><th>Cidade</th><td><?= htmlspecialchars($cidade) ?></td></tr>
                        <tr><th>Estado</th><td><?= htmlspecialchars($estado) ?></td></tr>
                        <tr><th>CEP / ZIP</th><td><?= htmlspecialchars($cep) ?></td></tr>
                        <tr><th>País</th><td><span class="badge bg-primary"><?= htmlspecialchars($pais ?: '?') ?></span></td></tr>
                    </table>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-6 mb-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header"><strong>Peso e Medidas</strong></div>
                <div class="card-body">
                    <table class="table table-sm mb-0">
                        <tr>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <span class="text-danger fw-bold">Não informado</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <span class="text-danger">Incompletas</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    </table>
                    ...
                    <?php if (!empty($itens)): ?>
                        <hr>
                        <h6 class="small text-muted mb-2">Itens do Pedido (Declaração Aduaneira)</h6>
                        <table class="table table-sm table-bordered" style="font-size:.8rem;">
                            <thead class="table-light">
                                <tr><th>Descrição</th><th>Qtd</th><th>Peso</th><th>Valor</th></tr>
                            </thead>
                            <tbody>
            ...
            <?php if ($etiqueta): ?>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
                        <strong><i class="fas fa-check-circle me-1"></i>Etiqueta Gerada</strong>
                        <button class="btn btn-sm btn-light" onclick="regerarEtiquetaShippo(<?= $pid ?>)"><i class="fas fa-redo me-1"></i>Regerar</button>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <table class="table table-sm mb-0">
                                    <tr><th style="width:130px;">Tracking</th><td>
                                    ...
                                    <tr><th>Carrier</th><td><?= htmlspecialchars((string) ($etiqueta['carrier'] ?? '-')) ?></td></tr>
                                    <tr><th>Serviço</th><td><?= htmlspecialchars((string) ($etiqueta['service_level'] ?? '-')) ?></td></tr>
                                    <tr><th>Valor</th><td><?= htmlspecialchars((string) ($etiqueta['rate_currency'] ?? 'USD')) ?> <?= number_format((float) ($etiqueta['rate_amount'] ?? 0), 2) ?></td></tr>
                                    <tr><th>Criada em</th><td><?= !empty($etiqueta['created_at']) ? date('d/m/Y H:i', strtotime((string) $etiqueta['created_at'])) : '-' ?></td></tr>
                                </table>
                            </div>
                            <div class="col-md-6 d-flex align-items-center justify-content-center">
                                ...
                                <?php else: ?>
                                    <span class="text-muted">PDF não disponível</span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <?php else: ?>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <strong><i class="fas fa-tags me-1"></i>Gerar Etiqueta Shippo</strong>
                    </div>
                    <div class="card-body">
                        <p class="text-muted small mb-3">
                            Clique no botão abaixo para consultar as opções de frete disponíveis via Shippo. 
                            Após receber as cotações, escolha a que preferir para gerar a etiqueta.
                        </p>

                        <button class="btn btn-primary" id="btnBuscarRates" onclick="buscarRatesShippo(<?= $pid ?>)">
                            <i class="fas fa-search me-1"></i>Buscar Opções de Frete
                        </button>

                        <!-- Container para exibir rates -->
                        <div id="ratesContainer" style="display:none;" class="mt-4">
                            <h6>Opções de frete disponíveis:</h6>
                        ...
                        <div id="ratesLoading" style="display:none;" class="mt-3">
                            ...
                            <span class="text-muted">Consultando opções na Shippo...</span>
                        </div>
                    </div>
                </div>
            <?php endif; ?>
...
async function buscarRatesShippo(pedidoId) {
    ...
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Consultando...'; }
    ...
    try {
        ...
        if (!data || !data.success) {
            setError(data.error || 'Falha ao buscar rates.');
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-search me-1"></i>Buscar Opções de Frete'; }
            return;
        }
        ...
        if (rates.length === 0) {
            setError('Nenhuma opção de frete disponível para este destino. Verifique os dados.');
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-search me-1"></i>Buscar Opções de Frete'; }
            return;
        }
            ...
            card.innerHTML = `
                <div class="card h-100 border rate-card" style="cursor:pointer;" onclick="confirmarRate('${rateId}', '${provider}', '${serviceName}', '${amount}', '${currency}')">
                    <div class="card-body text-center p-3">
                        ...
                        <div class="small text-muted">${days !== '?' ? days + ' dia(s) estimado(s)' : 'Prazo não informado'}</div>
                    </div>
                </div>
            `;
        ...
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-search me-1"></i>Buscar Novamente'; }
    } catch (e) {
        if (loading) loading.style.display = 'none';
...
async function confirmarRate(rateId, provider, service, amount, currency) {
    if (!confirm(`Confirmar compra da etiqueta?\n\nCarrier: ${provider}\nServiço: ${service}\nValor: ${currency} ${amount}\n\nEsta ação irá cobrar do seu saldo Shippo.`)) return;

    setError('');
    setSuccess('');
    const container = document.getElementById('ratesContainer');
    if (container) container.innerHTML = '<div class="text-center py-3"><div class="spinner-border text-primary" role="status"></div><div class="mt-2 text-muted">Gerando etiqueta...</div></div>';
    ...
    try {
        ...
        setSuccess(`
            <strong>Etiqueta gerada com sucesso!</strong><br>
            Tracking: <strong>${data.tracking_number || '-'}</strong><br>
            ${data.label_url ? '<a href="' + data.label_url + '" target="_blank" class="btn btn-sm btn-primary mt-2"><i class="fas fa-file-pdf me-1"></i>Baixar PDF</a>' : ''}
        `);
        if (container) container.style.display = 'none';
...
async function regerarEtiquetaShippo(pedidoId) {
    if (!confirm('Remover etiqueta atual do pedido #' + pedidoId + '?\nVocê poderá gerar uma nova em seguida.')) return;
...
$title = 'Shippo - Pedido #' . str_pad((string) $pid, 6, '0', STR_PAD_LEFT);
