...
function renderAdminScripts() {
    ...
    $__pConfigurado = isset($_SESSION['admin_pref_configurado']) ? (int)$_SESSION['admin_pref_configurado'] : -1;
    $__pIsAdmin = in_array(($_SESSION['usuario_perfil'] ?? ''), ['admin','suporte','vendedor'], true);
    $__pIsLogado = !empty($_SESSION['usuario_id']);
    if ($__pIsAdmin && $__pIsLogado && $__pConfigurado === -1) {
...
(function(){
    ...
    // Localized file input for standalone (non-layout) admin pages — replaces the browser's native "Escolher arquivo / Nenhum arquivo escolhido"
    $__fiChoose = json_encode(__('common.file.choose', 'Choose file'), JSON_UNESCAPED_UNICODE);
    $__fiChooseMulti = json_encode(__('common.file.choose_multiple', 'Choose files'), JSON_UNESCAPED_UNICODE);
    $__fiNone = json_encode(__('common.file.none_selected', 'No file selected'), JSON_UNESCAPED_UNICODE);
    $__fiCount = json_encode(__('common.file.count_selected', '{n} files selected'), JSON_UNESCAPED_UNICODE);
    echo '<style>.brz-file-i18n{display:inline-flex;align-items:center;gap:.5rem;flex-wrap:wrap;max-width:100%;}.brz-file-i18n>input[type="file"]{position:absolute!important;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap;border:0;}.brz-file-btn{display:inline-block;padding:.375rem .75rem;font-size:.875rem;line-height:1.5;color:#212529;background:#f8f9fa;border:1px solid #ced4da;border-radius:.375rem;cursor:pointer;white-space:nowrap;}.brz-file-btn:hover{background:#e9ecef;}.brz-file-name{color:#6c757d;font-size:.875rem;word-break:break-word;}</style>';
    echo '<script>(function(){var LC=' . $__fiChoose . ',LCM=' . $__fiChooseMulti . ',LN=' . $__fiNone . ',LCT=' . $__fiCount . ';function decorate(input){if(input.dataset.brzFileI18n==="1")return;if(input.classList.contains("purchase-group-file-input"))return;if(input.getAttribute("type")!=="file")return;input.dataset.brzFileI18n="1";var wrap=document.createElement("span");wrap.className="brz-file-i18n";input.parentNode.insertBefore(wrap,input);var btn=document.createElement("span");btn.className="brz-file-btn";btn.textContent=input.multiple?LCM:LC;var name=document.createElement("span");name.className="brz-file-name";name.textContent=LN;wrap.appendChild(input);wrap.appendChild(btn);wrap.appendChild(name);btn.addEventListener("click",function(){input.click();});input.addEventListener("change",function(){var f=input.files;if(!f||f.length===0){name.textContent=LN;}else if(f.length===1){name.textContent=f[0].name;}else{name.textContent=LCT.replace("{n}",String(f.length));}});}function decorateAll(root){(root||document).querySelectorAll("input[type=\"file\"]").forEach(decorate);}document.addEventListener("DOMContentLoaded",function(){decorateAll(document);});if(window.MutationObserver){var mo=new MutationObserver(function(muts){muts.forEach(function(m){m.addedNodes&&m.addedNodes.forEach(function(n){if(n.nodeType!==1)return;if(n.matches&&n.matches("input[type=\"file\"]"))decorate(n);else if(n.querySelectorAll)decorateAll(n);});});});document.addEventListener("DOMContentLoaded",function(){mo.observe(document.body,{childList:true,subtree:true});});}})();</script>';
}
