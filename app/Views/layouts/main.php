            ...
            if (is_array($decoded)) {
                foreach ($decoded as $item) {
                    if (is_string($item)) {
                        $src = trim($item);
                        if ($src === '') continue;
                        $layoutBanners[] = ['desktop' => $src, 'mobile' => ''];
                        continue;
                    }
                    if (is_array($item)) {
        ...
        <div class="container mt-3">
            <div id="siteHeaderBanners" class="carousel slide" data-bs-ride="carousel">
                            ...
                            <?php if (!empty($linkHref)): ?>
                                <a href="<?= htmlspecialchars($linkHref, ENT_QUOTES, 'UTF-8') ?>" style="display:block;">
                                    <picture>
                                        ...
                                        <img src="<?= htmlspecialchars($desktopSrc, ENT_QUOTES, 'UTF-8') ?>" class="d-block w-100" alt="Banner" style="max-height: 280px; object-fit: cover;">
                                    </picture>
                                </a>
                            <?php else: ?>
                                <picture>
                ...
                <?php if (count($layoutBanners) > 1): ?>
                    <button class="carousel-control-prev" type="button" data-bs-target="#siteHeaderBanners" data-bs-slide="prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Anterior</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#siteHeaderBanners" data-bs-slide="next">
