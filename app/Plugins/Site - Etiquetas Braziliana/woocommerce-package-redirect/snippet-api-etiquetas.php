...
function brz_api_package_pdf(WP_REST_Request $request) {
    ...
    // Retornar URL do PDF para download direto (o PDF é gerado no WP admin)
    $pdf_url = admin_url('post.php?post=' . $post_id . '&action=edit&generate_pdf=1');

    return new WP_REST_Response([
        'success' => true,
        'wp_post_id' => $post_id,
        'tracking_code' => $tracking_code,
        'pdf_admin_url' => $pdf_url,
        'message' => 'Use o endpoint /packages/pdf-download/' . $post_id . ' para download direto do PDF.',
    ], 200);
}
...
function brz_api_container_pdf(WP_REST_Request $request) {
    ...
    return new WP_REST_Response([
        ...
        'wp_post_id' => $post_id,
        'unit_code' => $unit_code,
        'message' => 'PDF do container disponível no admin do WordPress.',
    ], 200);
}
...
function brz_api_bill_pdf(WP_REST_Request $request) {
    ...
    return new WP_REST_Response([
        ...
        'wp_post_id' => $post_id,
        'cn38_code' => $cn38_code,
        'message' => 'PDF da fatura disponível no admin do WordPress.',
    ], 200);
}
