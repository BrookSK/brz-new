...
class PedidoEcommerce extends Model {
    ...
    public function adicionarHistoricoStatus($pedidoId, $statusAnterior, $statusNovo, $observacao, $usuarioId) {
        $stmt = $this->connection->prepare("
            INSERT INTO pedido_status_history (pedido_id, status_anterior, status_novo, observacao, alterado_por) 
            VALUES (:pedido_id, :status_anterior, :status_novo, :observacao, :alterado_por)
        ");
        ...
        $stmt->bindParam(':alterado_por', $usuarioId);
        $stmt->execute();
    }
