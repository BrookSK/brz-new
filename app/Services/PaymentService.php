...
class PaymentService {
    ...
    public function baixarEstoquePorPedido(int $pedidoId): void {
        ...
        try {
            ...
            // Idempotência persistente: se já houve baixa e ainda não houve estorno equivalente,
            // não baixar novamente (ex.: pago -> pendente -> pago).
            try {
                if ($this->isEstoqueBaixadoParaPedido($db, $pedidoId)) {
                    return;
                }
            } catch (\Exception $e) {
            }
            ...
            // Preparar suporte opcional a logs de movimentação
            $hasMov = false;
            $movCols = [];
            try {
                $st = $db->prepare('SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?');
                $st->execute(['estoque_movimentacao']);
                $hasMov = ((int) ($st->fetchColumn() ?: 0)) > 0;
            } catch (\Exception $e) {
                $hasMov = false;
            }
            if ($hasMov) {
                try {
                    $stCols = $db->query('DESCRIBE estoque_movimentacao');
                    $movCols = $stCols ? ($stCols->fetchAll(\PDO::FETCH_COLUMN) ?: []) : [];
                } catch (\Exception $e) {
                    $movCols = [];
                }
            }
            ...
            $stTotalInterno = null;
            if ($hasEstoqueInterno && $hasMov && is_array($movCols) && in_array('quantidade_anterior', $movCols, true) && in_array('quantidade_nova', $movCols, true)) {
                try {
                    $stTotalInterno = $db->prepare('SELECT COALESCE(SUM(COALESCE(quantidade,0)),0) FROM estoque_interno WHERE produto_id = ?');
                } catch (\Exception $e) {
                    $stTotalInterno = null;
                }
            }
            ...
            foreach ($itens as $it) {
                ...
                $qAnterior = null;
                $qNova = null;
                if ($hasMov && $stTotalInterno) {
                    try {
                        $stTotalInterno->execute([$pid]);
                        $qAnterior = (int) ($stTotalInterno->fetchColumn() ?: 0);
                    } catch (\Exception $e) {
                        $qAnterior = null;
                    }
                }
                ...
                if ($hasEstoqueInterno && $stLocs && $stUpdLoc) {
                    ...
                    if ($hasMov && $stTotalInterno && $qAnterior !== null) {
                        try {
                            $stTotalInterno->execute([$pid]);
                            $qNova = (int) ($stTotalInterno->fetchColumn() ?: 0);
                        } catch (\Exception $e) {
                            $qNova = null;
                        }
                    }

                    $this->registrarMovimentacaoEstoquePedido($db, $movCols, $pid, 'saida', $qtd, $qAnterior, $qNova, $pedidoId);
                    continue;
                }
                ...
                if ($hasMov) {
                    $this->registrarMovimentacaoEstoquePedido($db, $movCols, $pid, 'saida', $qtd, null, null, $pedidoId);
                }
            }

            // Marca a baixa como aplicada (para idempotência persistente)
            try {
                $this->registrarMarcadorBaixaEstoquePedido($db, $movCols, $pedidoId);
            } catch (\Exception $e) {
            }
        } catch (\Exception $e) {
            return;
    ...
    public function estornarEstoquePorPedido(int $pedidoId): void {
        $pedidoId = (int) $pedidoId;
        if ($pedidoId <= 0) {
            return;
        }
        ...
        try {
            ...
            // Só estorna se existir baixa líquida (baixa > estorno)
            $baixado = false;
            try {
                $baixado = $this->isEstoqueBaixadoParaPedido($db, $pedidoId);
            } catch (\Exception $e) {
                $baixado = false;
            }
            if (!$baixado) {
            ...
            $stInsInterno = null;
            if ($hasEstoqueInterno) {
                // Repor como uma nova entrada (não tentamos reverter FIFO por localização, pois pode não ser determinístico)
                try {
                    $stInsInterno = $db->prepare('INSERT INTO estoque_interno (produto_id, quantidade, data_compra, observacao) VALUES (?, ?, NOW(), ?)');
                } catch (\Exception $e) {
                    $stInsInterno = null;
                }
            }
            ...
            foreach ($itens as $it) {
                ...
                if ($hasEstoqueInterno && $stInsInterno) {
                    try {
                        $stInsInterno->execute([$pid, $qtd, 'Estorno por pedido #' . $pedidoId]);
                    } catch (\Exception $e) {
                    }
                } elseif ($stUpdProdutos) {
                    try {
                        $stUpdProdutos->execute([':qtd' => $qtd, ':pid' => $pid]);
                    } catch (\Exception $e) {
                    }
                ...
                if ($hasMov) {
                    $this->registrarMovimentacaoEstoquePedido($db, $movCols, $pid, 'estorno', $qtd, null, null, $pedidoId);
                }
            }

            // Marca o estorno (para permitir nova baixa sem duplicar)
            try {
                $this->registrarMarcadorEstornoEstoquePedido($db, $movCols, $pedidoId);
            } catch (\Exception $e) {
            }
        } catch (\Exception $e) {
            return;
    ...
    private function isEstoqueBaixadoParaPedido(\PDO $db, int $pedidoId): bool {
        try {
            $st = $db->prepare('SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?');
            $st->execute(['estoque_movimentacao']);
            $hasMov = ((int) ($st->fetchColumn() ?: 0)) > 0;
            ...
            if (!is_array($cols) || !in_array('motivo', $cols, true)) {
                return false;
            }

            $motivoBaixa = $this->getMotivoMarcadorBaixa($pedidoId);
            $motivoEstorno = $this->getMotivoMarcadorEstorno($pedidoId);

            $stB = $db->prepare('SELECT COUNT(*) FROM estoque_movimentacao WHERE motivo = ?');
            $stB->execute([$motivoBaixa]);
            $qB = (int) ($stB->fetchColumn() ?: 0);

            $stE = $db->prepare('SELECT COUNT(*) FROM estoque_movimentacao WHERE motivo = ?');
            $stE->execute([$motivoEstorno]);
            $qE = (int) ($stE->fetchColumn() ?: 0);
    ...
    private function getMotivoMarcadorBaixa(int $pedidoId): string {
        return 'BAIXA_ESTOQUE_PEDIDO:' . (int) $pedidoId;
    }

    private function getMotivoMarcadorEstorno(int $pedidoId): string {
        return 'ESTORNO_ESTOQUE_PEDIDO:' . (int) $pedidoId;
    }

    private function registrarMarcadorBaixaEstoquePedido(\PDO $db, array $movCols, int $pedidoId): void {
        if (empty($movCols) || !in_array('motivo', $movCols, true)) {
            return;
        }
        $this->registrarMovimentacaoEstoquePedido($db, $movCols, 0, 'baixa_pedido', 0, null, null, $pedidoId);
    }

    private function registrarMarcadorEstornoEstoquePedido(\PDO $db, array $movCols, int $pedidoId): void {
        if (empty($movCols) || !in_array('motivo', $movCols, true)) {
            return;
        }
        $this->registrarMovimentacaoEstoquePedido($db, $movCols, 0, 'estorno_pedido', 0, null, null, $pedidoId);
    }

    private function registrarMovimentacaoEstoquePedido(\PDO $db, array $movCols, int $produtoId, string $tipo, int $quantidade, ?int $qAnterior, ?int $qNova, int $pedidoId): void {
        if (empty($movCols) || !in_array('motivo', $movCols, true)) {
            return;
        }
        ...
        try {
            ...
            if (in_array('produto_id', $movCols, true)) {
                $cols[] = 'produto_id';
                $vals[] = ':produto_id';
                $bind[':produto_id'] = (int) $produtoId;
            }

            if (in_array('tipo_movimentacao', $movCols, true)) {
                $cols[] = 'tipo_movimentacao';
                $vals[] = ':tipo_movimentacao';
                $bind[':tipo_movimentacao'] = (string) $tipo;
            }

            if (in_array('quantidade', $movCols, true)) {
                $cols[] = 'quantidade';
                $vals[] = ':quantidade';
                $bind[':quantidade'] = (int) $quantidade;
            }

            if ($qAnterior !== null && in_array('quantidade_anterior', $movCols, true)) {
                $cols[] = 'quantidade_anterior';
                $vals[] = ':quantidade_anterior';
                $bind[':quantidade_anterior'] = (int) $qAnterior;
            }

            if ($qNova !== null && in_array('quantidade_nova', $movCols, true)) {
                $cols[] = 'quantidade_nova';
                $vals[] = ':quantidade_nova';
                $bind[':quantidade_nova'] = (int) $qNova;
            }

            $cols[] = 'motivo';
            $vals[] = ':motivo';

            if ($tipo === 'baixa_pedido') {
                $bind[':motivo'] = $this->getMotivoMarcadorBaixa($pedidoId);
            } elseif ($tipo === 'estorno_pedido') {
                $bind[':motivo'] = $this->getMotivoMarcadorEstorno($pedidoId);
            } else {
                $bind[':motivo'] = strtoupper($tipo) . ' pedido #' . (int) $pedidoId;
            }

            if (in_array('usuario_id', $movCols, true)) {
                $uid = null;
                try {
            ...
            $sql = 'INSERT INTO estoque_movimentacao (' . implode(', ', $cols) . ') VALUES (' . implode(', ', $vals) . ')';
            $st = $db->prepare($sql);
            $st->execute($bind);
        } catch (\Exception $e) {
            return;
