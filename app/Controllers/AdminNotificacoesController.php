...
class AdminNotificacoesController extends Controller {
    ...
    // =========================================================
    // HISTÓRICO DE NOTIFICAÇÕES DE PEDIDO (e-mail + WhatsApp) + REENVIO
    // =========================================================

    private function tabelaExisteNotif(\PDO $db, string $table): bool
    {
        try {
    ...
    /**
     * Listagem unificada de notificações enviadas (e-mail via email_event_log + WhatsApp via
     * webhook_disparos), com filtros e opção de reenvio.
    ...
    public function index(Request $request)
    {
        ...
        if (($filtroCanal === '' || $filtroCanal === 'email') && $this->tabelaExisteNotif($db, 'email_event_log')) {
            try {
                $where = [];
                $params = [];
                ...
                $st = $db->prepare("SELECT id, evento, to_email, subject, pedido_id, created_at FROM email_event_log {$whereSql} ORDER BY created_at DESC LIMIT 500");
                $st->execute($params);
                ...
                error_log('[NOTIF][LISTA] email_event_log: ' . $e->getMessage());
            }
        }

        // WhatsApp (webhook_disparos)
        if (($filtroCanal === '' || $filtroCanal === 'whatsapp') && $this->tabelaExisteNotif($db, 'webhook_disparos')) {
            try {
                $where = [];
                $params = [];
                ...
                $whereSql = !empty($where) ? ('WHERE ' . implode(' AND ', $where)) : '';
                $st = $db->prepare("SELECT wd.id, wd.pedido_id, wd.payload, wd.status, wd.response_code, wd.disparado_em FROM webhook_disparos wd {$whereSql} ORDER BY wd.disparado_em DESC LIMIT 500");
                $st->execute($params);
                foreach ($st->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                ...
                error_log('[NOTIF][LISTA] webhook_disparos: ' . $e->getMessage());
            }
        }
    ...
    /**
     * Reenvia a notificação (e-mail + WhatsApp) de um evento/pedido.
     * POST /admin/notificacoes/reenviar  Body JSON: { evento, pedido_id }
     */
