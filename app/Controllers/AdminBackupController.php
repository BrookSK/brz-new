...
<body>
    <div class="container-fluid">
                    ...
                    <div class="card-body">
                        <form method="POST" action="/admin/backup/agora" class="mb-3" id="backupNowForm">
                            <button type="submit" class="btn btn-success" id="backupNowBtn"' . ($isRunning ? ' disabled' : '') . '>
                                ...
                                <span class="backup-now-loading" style="display:none;"><span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>' . __('admin.backup.starting', 'Iniciando...') . '</span>
                            </button>
                        </form>
                        ...
                        <div id="backupProgressCard" class="' . ($isRunning ? '' : 'd-none') . '">
                            <div class="d-flex align-items-center gap-3 p-3 rounded" style="background:#eff6ff;border:1px solid #bfdbfe;">
                                ...
                                <div class="flex-grow-1">
                                    <div class="fw-semibold" style="color:#1e40af;">' . __('admin.backup.in_progress', 'Backup em andamento') . '</div>
                                    <div class="text-muted small">' . __('admin.backup.in_progress_help', 'Rodando em segundo plano. Você pode sair desta tela; o sino avisará quando terminar.') . '</div>
                                </div>
                        ...
                        <div class="text-muted small mt-2">' . __('admin.backup.actions_help', 'O backup gera um dump do banco (.sql) em segundo plano. Os arquivos do sistema são versionados via Git. A restauração atua apenas no banco.') . '</div>
                    </div>
                </div>
            </div>
        </div>';
        ...
        if (empty($runs)) {
            ...
            foreach ($runs as $r) {
                ...
                if ($status === 'ok') {
                    $statusBadge = '<span class="badge bg-success">' . __('admin.backup.status_ok', 'OK') . '</span>';
                } elseif ($status === 'processando') {
                    $statusBadge = '<span class="badge bg-info text-dark"><span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true" style="width:.7rem;height:.7rem;"></span>' . __('admin.backup.status_processing', 'Processando') . '</span>';
                } else {
                    $statusBadge = '<span class="badge bg-danger">' . __('admin.backup.status_error', 'Erro') . '</span>';
                }
    ...
    public function destravar(Request $request) {
        ...
        try {
            ...
            $_SESSION['message'] = __('admin.backup.unlocked_success', 'Backups presos foram destravados.');
            $_SESSION['message_type'] = 'success';
        } catch (\Throwable $e) {
            $_SESSION['message'] = __('admin.backup.unlock_error', 'Erro ao destravar: {msg}', ['msg' => $e->getMessage()]);
            $_SESSION['message_type'] = 'danger';
        }
