...
class DesapegoController extends Controller {
    ...
    public function index(Request $request) {
        ...
        // Verificar se a coluna desapego existe
        $cols = [];
        try {
        ...
        if (!in_array('desapego', $cols, true)) {
            $produtos = [];
        } else {
            // Buscar produtos marcados como desapego
            ...
            $select = ['p.id', 'p.' . $nameCol . ' AS name', 'p.' . $priceCol . ' AS price', 'p.' . $stockCol . ' AS stock'];
            if ($salePriceCol) $select[] = 'p.' . $salePriceCol . ' AS sale_price';
            if ($featuredCol) $select[] = 'p.' . $featuredCol . ' AS featured';
            if ($fotoCol) $select[] = 'p.' . $fotoCol . ' AS foto_principal';
            if (in_array('desapeguista_id', $cols, true)) $select[] = 'p.desapeguista_id';

            $sql = 'SELECT ' . implode(', ', $select) . ' FROM produtos p WHERE p.desapego = 1';
            ...
            if (in_array('oculto', $cols, true)) {
                $sql .= ' AND (p.oculto = 0 OR p.oculto IS NULL)';
            }
            if (in_array('status', $cols, true)) {
                $sql .= " AND LOWER(COALESCE(p.status,'')) NOT IN ('archived','inactive','inativo')";
            }
            // Somente com estoque > 0
            $sql .= ' AND p.' . $stockCol . ' > 0';
            ...
            try {
                ...
                error_log('[DESAPEGO] Erro na query: ' . $e->getMessage() . ' SQL: ' . $sql);
                $produtos = [];
            }
        }
