...
<title>' . htmlspecialchars(__('admin.cart_recovery.page_title', 'Recuperação de Carrinho - Admin'), ENT_QUOTES, 'UTF-8') . '</title>
...
<link href="/assets/css/dashboard-redesign.css" rel="stylesheet">';
        ...
        // Header
        echo '<header class="page-header"><div><h1 class="page-title">' . __('admin.cart_recovery.title', 'Recuperação de Carrinho') . '</h1><p class="page-subtitle">' . __('admin.cart_recovery.subtitle', 'Clientes que abandonaram o carrinho ou checkout nos últimos 7 dias') . '</p></div></header>';

        // KPIs
        echo '<div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:20px;">
<div class="kpi-card"><div><div class="kpi-label">' . __('admin.cart_recovery.kpi_abandoned', 'Abandonados') . '</div><div class="kpi-value">'.$stats['abandonados'].'</div><div class="kpi-subtext">' . __('admin.cart_recovery.kpi_abandoned_sub', 'Aguardando contato') . '</div></div><div class="kpi-icon" style="background:#FFE4E6;color:#9F1239;border-color:#FFE4E6;"><i class="bi bi-cart-x-fill"></i></div></div>
<div class="kpi-card"><div><div class="kpi-label">' . __('admin.cart_recovery.kpi_in_progress', 'Em Atendimento') . '</div><div class="kpi-value">'.$stats['em_atendimento'].'</div><div class="kpi-subtext">' . __('admin.cart_recovery.kpi_in_progress_sub', 'Sendo contactados') . '</div></div><div class="kpi-icon" style="background:#FEF3C7;color:#92400E;border-color:#FEF3C7;"><i class="bi bi-headset"></i></div></div>
<div class="kpi-card"><div><div class="kpi-label">' . __('admin.cart_recovery.kpi_recovered', 'Recuperados') . '</div><div class="kpi-value">'.$stats['recuperados'].'</div><div class="kpi-subtext">' . __('admin.cart_recovery.kpi_recovered_sub', 'Vendas salvas') . '</div></div><div class="kpi-icon" style="background:#D1FAE5;color:#065F46;border-color:#D1FAE5;"><i class="bi bi-check-circle-fill"></i></div></div>
<div class="kpi-card"><div><div class="kpi-label">' . __('admin.cart_recovery.kpi_lost', 'Perdidos') . '</div><div class="kpi-value">'.$stats['perdidos'].'</div><div class="kpi-subtext">' . __('admin.cart_recovery.kpi_lost_sub', 'Não converteram') . '</div></div><div class="kpi-icon"><i class="bi bi-x-circle-fill"></i></div></div>
</div>';

        // Filters
        $filtros = ['todos'=>__('admin.cart_recovery.filter_all', 'Todos'),'abandonado'=>__('admin.cart_recovery.kpi_abandoned', 'Abandonados'),'em_atendimento'=>__('admin.cart_recovery.kpi_in_progress', 'Em Atendimento'),'recuperado'=>__('admin.cart_recovery.kpi_recovered', 'Recuperados'),'perdido'=>__('admin.cart_recovery.kpi_lost', 'Perdidos')];
        ...
        // Table
        if (empty($registros)) {
            echo '<div class="section-card"><div class="section-body" style="text-align:center;padding:40px;"><i class="bi bi-cart-check" style="font-size:40px;color:#94A3B8;"></i><p style="color:#94A3B8;margin-top:12px;">' . __('admin.cart_recovery.empty', 'Nenhum carrinho abandonado encontrado.') . '</p></div></div>';
        } else {
...
<th style="padding:10px 14px;text-align:left;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_customer', 'Cliente') . '</th>
<th style="padding:10px 14px;text-align:center;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_items', 'Itens') . '</th>
<th style="padding:10px 14px;text-align:center;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_value', 'Valor') . '</th>
<th style="padding:10px 14px;text-align:center;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_abandoned_at', 'Abandonou em') . '</th>
<th style="padding:10px 14px;text-align:center;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_status', 'Status') . '</th>
<th style="padding:10px 14px;text-align:center;font-size:11px;text-transform:uppercase;color:#94A3B8;font-weight:700;">' . __('admin.cart_recovery.th_actions', 'Ações') . '</th>
</tr></thead><tbody>';

            $statusLabels = ['abandonado'=>[__('admin.cart_recovery.status_abandoned', 'Abandonado'),'#FFE4E6','#9F1239'],'em_atendimento'=>[__('admin.cart_recovery.kpi_in_progress', 'Em Atendimento'),'#FEF3C7','#92400E'],'recuperado'=>[__('admin.cart_recovery.status_recovered', 'Recuperado'),'#D1FAE5','#065F46'],'perdido'=>[__('admin.cart_recovery.status_lost', 'Perdido'),'#F1F5F9','#475569'],'nao_retornou'=>[__('admin.cart_recovery.status_not_returned', 'Não Retornou'),'#F1F5F9','#475569']];

            foreach ($registros as $r) {
                $st = $r['status'] ?? 'abandonado';
                $badge = $statusLabels[$st] ?? $statusLabels['abandonado'];
                $nome = htmlspecialchars($r['cliente_nome'] ?? (__('admin.cart_recovery.customer_prefix', 'Cliente #').$r['usuario_id']));
...
<small style="color:#94A3B8;">▼ ' . __('admin.cart_recovery.click_details', 'Clique para ver detalhes do carrinho') . '</small>
</div>
</td>
...
<option value="">' . __('admin.cart_recovery.opt_change', 'Alterar...') . '</option>
<option value="em_atendimento">' . __('admin.cart_recovery.kpi_in_progress', 'Em Atendimento') . '</option>
<option value="recuperado">' . __('admin.cart_recovery.status_recovered', 'Recuperado') . '</option>
<option value="perdido">' . __('admin.cart_recovery.status_lost', 'Perdido') . '</option>
...
async function atualizarStatusCarrinho(id, status){
    ...
    var pedidoId = 0;
    if(status === "recuperado"){
        pedidoId = parseInt(prompt("' . htmlspecialchars(__('admin.cart_recovery.prompt_order_id', 'ID do pedido recuperado (deixe 0 se não souber):'), ENT_QUOTES, 'UTF-8') . '") || "0");
    }
    ...
    else alert(d.error || "' . htmlspecialchars(__('admin.cart_recovery.error', 'Erro'), ENT_QUOTES, 'UTF-8') . '");
}
</script></body></html>';
    }
}
