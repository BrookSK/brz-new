...
class AdminCorreiosMundialController extends Controller {
    ...
    /**
     * Download em massa de etiquetas PACKET como ZIP
     */
    public function downloadEtiquetasMassa(Request $request) {
        $auth = new AuthService();
        $auth->requerPerfis(['admin', 'vendedor', 'suporte']);
        ...
        // Buscar etiquetas dos pedidos selecionados
        $placeholders = implode(',', array_fill(0, count($ids), '?'));
        $st = $this->connection->prepare("
            SELECT cpe.*, u.nome AS cliente_nome
            FROM correios_packet_etiquetas cpe
            LEFT JOIN pedidos p ON p.id = cpe.pedido_id
            LEFT JOIN usuarios u ON u.id = p.usuario_id
        ...
        if (empty($etiquetas)) {
            http_response_code(404);
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'error' => 'Nenhuma etiqueta encontrada']);
            return;
        }

        $tmpFile = tempnam(sys_get_temp_dir(), 'etiquetas_') . '.zip';
        $zip = new \ZipArchive();
        if ($zip->open($tmpFile, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) !== true) {
            http_response_code(500);
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'error' => 'Falha ao criar arquivo ZIP']);
            return;
        }
        ...
        foreach ($etiquetas as $etiqueta) {
            ...
            $clienteNome = trim((string) ($etiqueta['cliente_nome'] ?? ''));
            if ($tracking === '' || $pedidoId <= 0) continue;

            try {
                $pedido = $pedidoModel->getComDetalhes($pedidoId);
                if (!is_array($pedido) || empty($pedido['id'])) continue;

                $pdfContent = $this->gerarEtiquetaPdfContent($etiqueta, $pedido);
                if ($pdfContent === '') continue;

                // Nome do arquivo: #745 - Nome do Cliente.pdf
                $safeName = preg_replace('/[\\\\\/\:\*\?\"\<\>\|]/', '', $clienteNome);
                $pdfFilename = '#' . $pedidoId . ' - ' . $safeName . '.pdf';
        ...
        header('Content-Disposition: attachment; filename="etiquetas_packet_' . date('Y-m-d_His') . '.zip"');
        header('Content-Length: ' . strlen($zipContent));
        echo $zipContent;
    }

    /**
     * Gera o conteúdo PDF de uma etiqueta (reutiliza a lógica de etiquetaPdf mas retorna string)
     */
    private function gerarEtiquetaPdfContent(array $etiqueta, array $pedido): string {
        $tracking = (string) ($etiqueta['tracking_number'] ?? '');
        $pedidoId = (int) ($etiqueta['pedido_id'] ?? 0);
        ...
        foreach ($pedidoItems as $pi) {
            ...
            $q = (int) ($pi['quantidade'] ?? 0);
            if ($q <= 0) $q = 1;
            $w = isset($pi['peso_kg']) && is_numeric($pi['peso_kg']) ? (float) $pi['peso_kg'] : 0.0;
            if ($w > 0) $totalWeightKg += ($w * $q);
        }
        $totalWeightKg = round($totalWeightKg, 2);
        ...
        foreach ($items as $it) {
            if (!is_array($it)) continue;
            $itemsAll[] = ['hsCode' => (string) ($it['hsCode'] ?? ''), 'description' => (string) ($it['description'] ?? ''), 'quantity' => (int) ($it['quantity'] ?? 0), 'value' => (float) ($it['value'] ?? 0)];
        }

        // Gerar HTML simplificado da etiqueta
        $h = fn($v) => htmlspecialchars((string) $v, ENT_QUOTES, 'UTF-8');
        $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;font-size:10px;margin:5mm;}table{width:100%;border-collapse:collapse;}td,th{border:1px solid #000;padding:3px 5px;}.barcode{text-align:center;}.header{text-align:center;font-weight:bold;font-size:12px;margin-bottom:5px;}</style></head><body>';
        $html .= '<div class="header">' . $h($modalityDescription) . '</div>';
        $html .= '<table><tr><td colspan="2" class="barcode"><img src="' . $barTrackingImg . '" style="height:50px;"><br><strong>' . $h($tracking) . '</strong></td></tr>';
        $html .= '<tr><td><strong>De:</strong> ' . $h($senderName) . '<br>' . $h($senderAddr) . '<br>' . $h($senderCity) . '/' . $h($senderState) . ' ' . $h($senderZip) . '</td>';
        $html .= '<td><strong>Para:</strong> ' . $h($destName) . '<br>' . $h($destAddr) . ($destComp !== '' ? ' - ' . $h($destComp) : '') . '<br>' . $h($destCity) . '/' . $h($destState) . ' CEP: ' . $h($recipientZipPretty) . '</td></tr>';
        $html .= '<tr><td><strong>Ref:</strong> ' . $h($orderRef) . '</td><td><strong>Peso:</strong> ' . number_format($totalWeightKg, 2) . ' kg | <strong>Total:</strong> USD ' . number_format($totalUsd, 2) . '</td></tr>';
        $html .= '<tr><td colspan="2"><strong>Itens:</strong><br>';
        foreach ($itemsAll as $it) {
            $html .= '- ' . $h($it['description']) . ' (x' . $it['quantity'] . ') USD ' . number_format($it['value'], 2) . '<br>';
        }
